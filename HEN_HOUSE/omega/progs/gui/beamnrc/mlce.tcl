...
###############################################################################
#
#  EGSnrc BEAMnrc graphical user interface: MLCE
...
proc read_MLCE { fileid id } {
	...
	tk_dialog .error "Read Error" "Error in the input file when starting\
		to read MLCE $cm_ident($id).  The inputs don't begin where\
...
proc edit_MLCE { id zmax } {
    global cmval GUI_DIR helvfont cm_ident ngroups help_mlce_text
    global default1

    catch { destroy .mlce$id }
    toplevel .mlce$id
    wm title .mlce$id "Edit MLCE, CM\#$id"
    # MAIN WINDOW - PUT ON THE MAIN STUFF; A >> BUTTON BY MLCE.
    frame .mlce$id.top -bd 4
    set top .mlce$id.top
    ...
    # MLCE_macros.mortran:REPLACE {$MAXLEAF} WITH {64}
    # read the macro file for CM MLCE to get defaults and put a label on the
    # main edit window.  Has to be different from the others because there's
    # no MLCE in the default name.

    global omega helvfont
    set filename $omega/beamnrc/CMs/MLCE_macros.mortran
    set default1 {}
    ...
    end_param_menu_mlce $top.endparam.endtype.inp $id
...
#now get ecut,pcut,materials,etc
    ...
    frame .mlce$id.bottom
    frame .mlce$id.bottom.left
    frame .mlce$id.bottom.right

    label .mlce$id.bottom.left.lab -text "Opening(s) and air gaps" \
                       -font $helvfont
    pack .mlce$id.bottom.left.lab -side top -pady 5
    add_ecut .mlce$id.bottom.left.f0 $id,13,0
    add_pcut .mlce$id.bottom.left.f1 $id,13,1
    add_dose .mlce$id.bottom.left.f2 $id,13,2
    add_latch .mlce$id.bottom.left.f3 $id,13,3
    add_material .mlce$id.bottom.left.f4 $id,14

    label .mlce$id.bottom.right.lab -text "Collimator leaves" -font $helvfont
    pack .mlce$id.bottom.right.lab -side top -pady 5
    add_ecut .mlce$id.bottom.right.f0 $id,15,0
    add_pcut .mlce$id.bottom.right.f1 $id,15,1
    add_dose .mlce$id.bottom.right.f2 $id,15,2
    add_latch .mlce$id.bottom.right.f3 $id,15,3
    add_material .mlce$id.bottom.right.f4 $id,16

    pack .mlce$id.bottom.left -side left -anchor n -padx 10
    pack .mlce$id.bottom.right -side right -anchor n -padx 10

    pack .mlce$id.bottom -side top -pady 5

    frame .mlce$id.buts -bd 4
    button .mlce$id.buts.okb -text "OK" -command "destroy .mlce$id"\
	    -relief groove -bd 8
    button .mlce$id.buts.helpb -text "Help" -command\
	    "help_gif .mlce$id.help {$help_mlce_text} help_mlce"\
	    -relief groove -bd 8
#don't have a preview yet, get it working after course, will call show_MLCE
    button .mlce$id.buts.prev -text "Preview" -command\
                          "show_MLCE $id" -relief groove -bd 8
#	    { tk_dialog .nopreview "No Preview" \
#             "No preview available yet." info 0 OK } -relief groove -bd 8
    pack .mlce$id.buts.helpb .mlce$id.buts.okb .mlce$id.buts.prev -side left -padx 10
    pack .mlce$id.buts -pady 10
}

proc end_param_menu_mlce { w id } {
    global cmval

    set inw .mlce$id.top.endparam.focrad.zr.label
    set inx .mlce$id.top.endparam.focrad.cil.label
    set iny .mlce$id.top.endparam.focrad.cil.inp
    if { $cmval($id,10)==1 } {
...
proc define_mlce_leaves { id } {
    global cmval helvfont ngroups to from help_mlce_text
    ...
    catch { destroy .mlce$id.child }
    toplevel .mlce$id.child
    set top .mlce$id.child
    ...
    message $w.message -text "Specify the minimum and maximum $orientation\
	    ...
	    each leaf in the MLCE by groups here.  Add a row to start a\
    ...
    pack $w
    frame $top.b
    button $top.b.addb -text "Add a row" -command "add_mlce_row $id"\
	    -relief groove -bd 8
    button $top.b.delb -text "Delete last row"\
            -command "del_mlce_row $id" -relief groove -bd 8
    button $top.b.okb -text "OK" -command "save_mlce $id" -relief groove -bd 8
    button $top.b.helpb -text "Help" -command\
	    "help_gif .mlce$id.child.help {$help_mlce_text} help_mlce"\
	    -relief groove -bd 8
...
proc save_mlce { id } {
    ...
    destroy .mlce$id.child

}

proc add_mlce_row { id } {
    global ngroups from to cmval

    set w .mlce$id.child.f
...
proc show_MLCE { id } {
    ...
    catch { destroy .mlce$id.show }
    toplevel .mlce$id.show
    wm title .mlce$id.show "Preview"
    ...
    frame .mlce$id.show.frm

    draw_MLCE $id

    frame .mlce$id.show.buts
    button .mlce$id.show.buts.range -text "Change plot properties" -command\
	    "change_cm_range $id .mlce$id.show xyz" -relief groove -bd 8
    button .mlce$id.show.buts.print1 -text "Print xy..." -command\
	    "print_canvas .mlce$id.show.frm.can.one 450 450" -relief groove -bd 8
    button .mlce$id.show.buts.print2 -text "Print xz..." -command\
	    "print_canvas .mlce$id.show.frm.can.two 450 450" -relief groove -bd 8
    button .mlce$id.show.buts.print3 -text "Print yz..." -command\
	    "print_canvas .mlce$id.show.frm.can.three 450 450" -relief groove -bd 8
    button .mlce$id.show.buts.done -text "Done" -command\
	    "destroy .mlce$id.show" -relief groove -bd 8
    pack .mlce$id.show.buts.range .mlce$id.show.buts.print1\
	    .mlce$id.show.buts.print2 .mlce$id.show.buts.print3\
	    .mlce$id.show.buts.done -side left -padx 10
    pack .mlce$id.show.buts -side bottom -anchor s -pady 15
}
...
proc draw_MLCE { id } {
    ...
    catch { destroy .mlce$id.show.frm.can }
	...
	catch { destroy .mlce$id.show.frm.scrlz }
	catch { destroy .mlce$id.show.frm.scrlx }
	scrollbar .mlce$id.show.frm.scrlz -command\
		".mlce$id.show.frm.can yview"
	scrollbar .mlce$id.show.frm.scrlx -command\
		".mlce$id.show.frm.can xview" -orient horizontal
	pack .mlce$id.show.frm.scrlz -side right -fill y
	pack .mlce$id.show.frm.scrlx -side bottom -fill x
	canvas .mlce$id.show.frm.can -width $winwidth -height $winheight\
		-yscrollcommand ".mlce$id.show.frm.scrlz set"\
		-xscrollcommand ".mlce$id.show.frm.scrlx set"\
		-scrollregion "0 0 $scrlwidth $scrlheight"
    } else {
	canvas .mlce$id.show.frm.can -width $scrlwidth -height $scrlheight
    }
    ...
    canvas .mlce$id.show.frm.can.one -width $canwidth -height $canwidth

    add_MLCE_xy $id $xscale $yscale $xrange(0) $yrange(0)\
	    $l $m .mlce$id.show.frm.can.one

    if {$transmod==0} {
    # Put some text in the upper left corner, just to fill the gap.
    .mlce$id.show.frm.can create text 225 225 -text "The preview of the\
            leaf sides\
...
WARNING: Leaves shown are simply translated to be SPACE cm apart\
    ...
    set curx 0
    set cury 0
    label .mlce$id.show.frm.can.one.xy -text \
	    [format "(%6.3f, %6.3f)" $curx $cury] -bg white\
	    -bd 5 -width 16 -font $helvfont
    .mlce$id.show.frm.can.one create window [expr $l/2+$width]\
	    [expr 2*$m+$width] -window .mlce$id.show.frm.can.one.xy
    bind .mlce$id.show.frm.can.one <Motion> {
    ...
    coverup $l $m $width .mlce$id.show.frm.can.one

    add_axes xy $cm_ticks($id,x) $cm_ticks($id,y) $l $m $width $xrange(0)\
	    $xrange(1) $yrange(0) $yrange(1)\
	    $xscale $yscale .mlce$id.show.frm.can.one

    .mlce$id.show.frm.can create window [expr $canwidth+$canwidth/2.0]\
	    [expr $canwidth/2.0] -window .mlce$id.show.frm.can.one

    # XZ, lower left: if leaves // to y, show leaf sides,
    # else if leaves // to x, show only one leaf at y=0 (leaf ends).

    canvas .mlce$id.show.frm.can.two -width $canwidth -height $canwidth

    add_air $id .mlce$id.show.frm.can.two $xrange(0) $zrange(0)\
	    $xscale $zscale $l $m

    if $cmval($id,2)==0 {
	add_MLCE_sides $id $xscale $zscale $xrange(0) $zrange(0) $zrange(1)\
		$l $m .mlce$id.show.frm.can.two
    } else {
        add_MLCE_ends $id $xscale $zscale $xrange(0) $zrange(0) $zrange(1)\
		$l $m .mlce$id.show.frm.can.two
    }

    coverup $l $m $width .mlce$id.show.frm.can.two

    label .mlce$id.show.frm.can.two.xy -text \
	    [format "(%6.3f, %6.3f)" $curx $cury] -bg white\
	    -bd 5 -width 16 -font $helvfont
    .mlce$id.show.frm.can.two create window [expr $l/2+$width]\
	    [expr 2*$m+$width] -window .mlce$id.show.frm.can.two.xy
    bind .mlce$id.show.frm.can.two <Motion> {
    ...
    add_axes xz $cm_ticks($id,x) $cm_ticks($id,z) $l $m $width $xrange(0)\
	    ...
	    $xscale $zscale .mlce$id.show.frm.can.two

    .mlce$id.show.frm.can create window [expr $canwidth/2.0]\
	    [expr $canwidth+$canwidth/2.0] -window .mlce$id.show.frm.can.two

    # YZ, lower right

    canvas .mlce$id.show.frm.can.three -width $canwidth -height $canwidth

    add_air $id .mlce$id.show.frm.can.three $yrange(0) $zrange(0)\
	    $yscale $zscale $l $m

    if $cmval($id,2)==0 {
	add_MLCE_ends $id $yscale $zscale $yrange(0) $zrange(0) $zrange(1)\
		$l $m .mlce$id.show.frm.can.three
    } else {
	add_MLCE_sides $id $yscale $zscale $yrange(0) $zrange(0) $zrange(1)\
		$l $m .mlce$id.show.frm.can.three
    }

    coverup $l $m $width .mlce$id.show.frm.can.three

    label .mlce$id.show.frm.can.three.xy -text \
	    [format "(%6.3f, %6.3f)" $curx $cury] -bg white\
	    -bd 5 -width 16 -font $helvfont
    .mlce$id.show.frm.can.three create window [expr $l/2+$width]\
	    [expr 2*$m+$width] -window .mlce$id.show.frm.can.three.xy
    bind .mlce$id.show.frm.can.three <Motion> {
    ...
    add_axes yz $cm_ticks($id,y) $cm_ticks($id,z) $l $m $width $yrange(0)\
	    ...
	    $yscale $zscale .mlce$id.show.frm.can.three

    .mlce$id.show.frm.can create window [expr $canwidth+$canwidth/2.0]\
	    [expr $canwidth+$canwidth/2.0] -window .mlce$id.show.frm.can.three

    pack .mlce$id.show.frm.can -side top -anchor n
    pack .mlce$id.show.frm -side top -anchor n
}

proc add_MLCE_xy {id xscale yscale xmin ymin l m parent_w} {

    global cmval colorlist medium nmed colornum transmod
...
proc add_MLCE_ends {id xscale zscale xmin zmin zmax l m parent_w} {
...
proc add_MLCE_sides {id yscale zscale ymin zmin zmax l m parent_w} {
