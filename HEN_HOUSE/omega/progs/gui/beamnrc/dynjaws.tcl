...
###############################################################################
#
#  EGSnrc BEAMnrc graphical user interface: DYNJAWS
...
proc read_DYNJAWS { fileid id } {
	...
	tk_dialog .error "Read Error" "Error in the input file when starting\
		to read DYNJAWS $cm_ident($id).  The inputs don't begin where\
...
proc edit_DYNJAWS { id zmax } {
    global cmval GUI_DIR omega helvfont help_dynjaws_text values sync_file
    global inp_file_dir

    catch { destroy .dynjaws$id }
    toplevel .dynjaws$id
    wm title .dynjaws$id "Edit DYNJAWS, CM\#$id"
    # MAIN WINDOW - PUT ON THE MAIN STUFF; A >> BUTTON BY NJAWS.
    frame .dynjaws$id.frm -bd 4
    set top .dynjaws$id.frm

    label $top.mainlabel -text "DYNJAWS" -font $helvfont
    pack $top.mainlabel -pady 10

    #JAWS_macros.mortran:REPLACE {$MAX_N_$JAWS} WITH {10}
    get_1_default DYNJAWS $top "maximum number of paired bars or jaws"
    ...
    frame $top.buts -bd 4
    button $top.buts.okb -text "OK" -command "destroy .dynjaws$id"\
	    -relief groove -bd 8
    button $top.buts.helpb -text "Help" -command \
	    "help_gif .dynjaws$id.help {$help_dynjaws_text} help_jaws"\
	    -relief groove -bd 8
    button $top.buts.prev -text "Preview" -command \
	    "show_DYNJAWS $id" -relief groove -bd 8
    pack $top.buts.helpb $top.buts.okb $top.buts.prev -padx 10 -side left
    pack $top.buts -pady 5
    pack $top
}

proc define_dynjaws { id } {
    global cmval helvfont values help_dynjaws_text nmed medium med_per_column

    catch { destroy .dynjaws$id.child }
    toplevel .dynjaws$id.child
    set top .dynjaws$id.child
...
size and SSD" -command "quikdyndef $id" -relief groove -bd 8
    ...
    pack $w
    frame $top.b
    button $top.b.okb -text "OK" -command "destroy .dynjaws$id.child"\
	    -relief groove -bd 8
    button $top.b.helpb -text "Help" -command\
	    "help_gif .dynjaws$id.child.help {$help_dynjaws_text} help_jaws"\
	    -relief groove -bd 8
...
proc show_DYNJAWS { id } {
    global cmval helvfont medium nmed xrange yrange zrange med cm_ticks sync_file

    catch { destroy .dynjaws$id.show }
    toplevel .dynjaws$id.show
    wm title .dynjaws$id.show "Preview"
    ...
    if {"$cmval($id,2,1)"=="1" || "$cmval($id,2,1)"=="2"} {
     ...
     if { [file readable $sync_file($id)] } {
       ...
       tk_dialog tk_dialog .dynjaws$id.nojawfile1 "No jaw opening file" "No\
    ...
    if [catch { draw_DYNJAWS $id }]==1 {
	destroy .dynjaws$id.show
	tk_dialog .dynjaws$id.error "Preview error" "There was an error in\
    ...
    if {"$cmval($id,2,1)"=="1"} {
      ...
      message .dynjaws$id.show.message -text "NOTE: Jaw openings displayed \
         are for the first field in a series of dynamic fields." -width 500
      pack .dynjaws$id.show.message -side top -pady 5
    ...
    frame .dynjaws$id.show.buts
    button .dynjaws$id.show.buts.range -text "Change plot properties" -command\
	    "change_cm_range $id .dynjaws$id.show xyz" -relief groove -bd 8
    button .dynjaws$id.show.buts.print1 -text "Print xz..." -command\
	    "print_canvas .dynjaws$id.show.can.one 450 450" -relief groove -bd 8
    button .dynjaws$id.show.buts.print2 -text "Print yz..." -command\
	    "print_canvas .dynjaws$id.show.can.two 450 450" -relief groove -bd 8
    button .dynjaws$id.show.buts.done -text "Done" -command\
	    "destroy .dynjaws$id.show" -relief groove -bd 8
    pack .dynjaws$id.show.buts.range .dynjaws$id.show.buts.print1\
	    .dynjaws$id.show.buts.print2 .dynjaws$id.show.buts.done\
	    -side left -padx 10
    pack .dynjaws$id.show.buts -side bottom -anchor s -pady 15
}

proc draw_DYNJAWS { id } {
    global helvfont zrange yrange xrange zscale yscale xscale l m
    global cm_ticks

    catch { destroy .dynjaws$id.show.can }

    # put the canvas up
    canvas .dynjaws$id.show.can -width 900 -height 400
    ...
    canvas .dynjaws$id.show.can.one -width $canwidth -height $canwidth

    add_air $id .dynjaws$id.show.can.one $xrange(0) $zrange(0)\
	    $xscale $zscale $l $m

    add_DYNJAWS $id $xscale $zscale $xrange(0) $zrange(0)\
	    $l $m .dynjaws$id.show.can.one

    coverup $l $m $width .dynjaws$id.show.can.one

    set curx 0
    set cury 0
    label .dynjaws$id.show.can.one.xy -text [format "(%6.3f, %6.3f)" $curx $cury]\
	    -bg white -bd 5 -width 16 -font $helvfont
    .dynjaws$id.show.can.one create window [expr $l/2+$width] [expr 2*$m+$width-10]\
	    -window .dynjaws$id.show.can.one.xy
    bind .dynjaws$id.show.can.one <Motion> {
    ...
    add_axes xz $cm_ticks($id,x) $cm_ticks($id,z) $l $m $width $xrange(0)\
	    ...
	    $xscale $zscale .dynjaws$id.show.can.one

    .dynjaws$id.show.can create window [expr $canwidth/2.0]\
	    [expr $canwidth/2.0] -window .dynjaws$id.show.can.one

    # right canvas, YZ cross-section

    canvas .dynjaws$id.show.can.two -width $canwidth -height $canwidth

    add_air $id .dynjaws$id.show.can.two $yrange(0) $zrange(0)\
	    $yscale $zscale $l $m

    add_DYNJAWS_yz $id $yscale $zscale $yrange(0) $zrange(0)\
	    $l $m .dynjaws$id.show.can.two

    coverup $l $m $width .dynjaws$id.show.can.two

    label .dynjaws$id.show.can.two.xy -text [format "(%6.3f, %6.3f)" $curx $cury]\
	    -bg white -bd 5 -width 16 -font $helvfont
    .dynjaws$id.show.can.two create window [expr $l/2+$width] [expr 2*$m+$width-10]\
	    -window .dynjaws$id.show.can.two.xy
    bind .dynjaws$id.show.can.two <Motion> {
    ...
    add_axes yz $cm_ticks($id,y) $cm_ticks($id,z) $l $m $width $yrange(0)\
	    ...
	    $yscale $zscale .dynjaws$id.show.can.two

    .dynjaws$id.show.can create window [expr $canwidth+$canwidth/2.0]\
	    [expr $canwidth/2.0] -window .dynjaws$id.show.can.two

    pack .dynjaws$id.show.can -side top -anchor n
}

proc add_DYNJAWS {id xscale zscale xmin zmin l m parent_w} {
...
proc add_DYNJAWS_yz {id xscale zscale xmin zmin l m parent_w} {
    ...
    if {"$cmval($id,2,1)"=="1" || "$cmval($id,2,1)"=="2"} {
     ...
     if { [file readable $sync_file($id)] } {
       set fileid [open $sync_file($id) "r"]
       #first three lines are no. of fields and index of first field
       gets $fileid data
       gets $fileid data
       gets $fileid data
       for {set i 1} {$i<=$cmval($id,2,0)} {incr i} {
       ...
       tk_dialog tk_dialog .dynjaws$id.nojawfile2 "No jaw opening file" "No\
    jaw opening file has been specified or the specified file does not exist\
    or is not readable.  This is required for showing jaw openings in the\
    preview if dynamic or step-and-shoot fields are specified." error 0 OK
       return
...
# jaws using field dimensions, SSD and Z from which SSD is measured
    ...
    catch { destroy .dynjaws$id.child.qkdef }
    toplevel .dynjaws$id.child.qkdef
    set top .dynjaws$id.child.qkdef
    ...
    button $top.but.update -text "Update x/y coords." -command "quikdyncalc $id"\
            -relief groove -bd 8
    button $top.but.quit -text "Quit" -command "destroy .dynjaws$id.child.qkdef"\
            -relief groove -bd 8
...
#   and updates the coordinates as displayed in the jaws definition window
    ...
    set dynjawlist [.dynjaws$id.child.qkdef.u.r.list.num curselection]

    if { $dynjawlist=="" } {
       tk_dialog .dynjaws$id.warning1 "No jaws selected" "You must select at\
       least 1 set of jaws to apply this field size & SSD to." warning 0 OK
       return
    }

    if { $ssd($id)=="" || $fmin($id)=="" || $fmin($id)=="" || $fieldfocus($id)==""} {
       tk_dialog .dynjaws$id.warning2 \
...
[expr ($cmval($id,4,1,$j)-$fieldfocus($id))*double($fmin($id))/double($ssd($id))
    ...
    if { $ierror > 0 } {
       tk_dialog .dynjaws$id.warning4 "Front and back of jaws not defined"\
