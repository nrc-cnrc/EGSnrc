...
###############################################################################
#
#  EGSnrc BEAMnrc graphical user interface: SYNCVMLC
...
proc read_SYNCVMLC { fileid id } {
	...
	tk_dialog .error "Read Error" "Error in the input file when starting\
		to read SYNCVMLC $cm_ident($id).  The inputs don't begin where\
...
proc edit_SYNCVMLC { id zmax } {
    global cmval GUI_DIR helvfont cm_ident ngroups help_syncvmlc_text
    global nleaf default1 nmed medium values sync_file inp_file_dir med_per_column

    catch { destroy .syncvmlc$id }
    toplevel .syncvmlc$id
    wm title .syncvmlc$id "Edit SYNCVMLC, CM\#$id"
    # MAIN WINDOW - PUT ON THE MAIN STUFF; A >> BUTTON BY SYNCVMLC.
    frame .syncvmlc$id.top -bd 4
    set top .syncvmlc$id.top
    ...
    # SYNCVMLC_macros.mortran:REPLACE {$MAXLEAF} WITH {64}
    # read the macro file for CM SYNCVMLC to get defaults and put a label on the
    # main edit window.  Has to be different from the others because there's
    # no SYNCVMLC in the default name.

    global omega helvfont
    set filename $omega/beamnrc/CMs/SYNCVMLC_macros.mortran
    set default1 {}
    ...
    button $top.inp2.right.help -bitmap @$GUI_DIR/help_icon.xbm \
            -command "help_ignoregaps_syncvmlc [winfo parent $top.inp2.right]"
    ...
    end_param_menu_syncvmlc $top.endparam.inp17.inp $id
    ...
    button $top.openings.inps.static.but -text "Define leaf openings >>"\
             -command "define_syncvmlc_openings $id"
    if { $cmval($id,2,2)==1 | $cmval($id,2,2)==2 } {
...
#now get ecut,pcut,materials,etc
    ...
    frame .syncvmlc$id.bottom -bd 4

    frame .syncvmlc$id.bottom.ecut
    button .syncvmlc$id.bottom.ecut.b -text "?" -command \
           "help_ecut .syncvmlc$id.bottom.ecut"
    label .syncvmlc$id.bottom.ecut.lab -text \
          "Electron cutoff energy (default ECUTIN) (MeV)"
    pack .syncvmlc$id.bottom.ecut.b -anchor w -side left
    pack .syncvmlc$id.bottom.ecut.lab -side left -padx 5
    grid configure .syncvmlc$id.bottom.ecut -column 0 -row 1 -sticky e

    frame .syncvmlc$id.bottom.pcut
    button .syncvmlc$id.bottom.pcut.b -text "?" \
          -command "help_pcut .syncvmlc$id.bottom.pcut"
    label .syncvmlc$id.bottom.pcut.lab -text \
          "Photon cutoff energy (default PCUTIN) (MeV)"
    pack .syncvmlc$id.bottom.pcut.b -anchor w -side left
    pack .syncvmlc$id.bottom.pcut.lab -side left -padx 5
    grid configure .syncvmlc$id.bottom.pcut -column 0 -row 2 -sticky e

    frame .syncvmlc$id.bottom.dose
    button .syncvmlc$id.bottom.dose.b -text "?" \
         -command "help_dose .syncvmlc$id.bottom.dose"
    label .syncvmlc$id.bottom.dose.lab -text "Dose zone (0 for no scoring)"
    pack .syncvmlc$id.bottom.dose.b -anchor w -side left
    pack .syncvmlc$id.bottom.dose.lab -side left -padx 5
    grid configure .syncvmlc$id.bottom.dose -column 0 -row 3 -sticky e

    frame .syncvmlc$id.bottom.latch
    button .syncvmlc$id.bottom.latch.b -text "?" \
        -command "help_latch .syncvmlc$id.bottom.latch"
    label .syncvmlc$id.bottom.latch.lab -text "Associate with LATCH bit"
    pack .syncvmlc$id.bottom.latch.b -anchor w -side left
    pack .syncvmlc$id.bottom.latch.lab -padx 5 -side left
    grid configure .syncvmlc$id.bottom.latch -column 0 -row 4 -sticky e

    frame .syncvmlc$id.bottom.med
    button .syncvmlc$id.bottom.med.b -text "?" \
        -command "help_material .syncvmlc$id.bottom.med"
    label .syncvmlc$id.bottom.med.lab -text "Material"
    pack .syncvmlc$id.bottom.med.b -anchor w -side left
    pack .syncvmlc$id.bottom.med.lab -padx 5 -side left
    grid configure .syncvmlc$id.bottom.med -column 0 -row 5 -sticky e

    label .syncvmlc$id.bottom.t0 -text "Opening(s) + air gaps" \
                       -font $helvfont
    label .syncvmlc$id.bottom.t1 -text "Collimator leaves" \
                       -font $helvfont
    label .syncvmlc$id.bottom.t2 -text "Driving screw holes" \
                       -font $helvfont
    grid configure .syncvmlc$id.bottom.t0 -column 1 -row 0
    grid configure .syncvmlc$id.bottom.t1 -column 2 -row 0
    grid configure .syncvmlc$id.bottom.t2 -column 3 -row 0
    ...
    foreach i {15 17 19} {
      ...
      entry .syncvmlc$id.bottom.ecut$k -textvariable cmval($id,$i,0) -width 20
      grid configure .syncvmlc$id.bottom.ecut$k -column $k -row 1
    }
    ...
    foreach i {15 17 19} {
      ...
      entry .syncvmlc$id.bottom.pcut$k -textvariable cmval($id,$i,1) -width 20
      grid configure .syncvmlc$id.bottom.pcut$k -column $k -row 2
    }
    ...
    foreach i {15 17 19} {
      incr k
      entry .syncvmlc$id.bottom.dose$k -textvariable cmval($id,$i,2) -width 20
      grid configure .syncvmlc$id.bottom.dose$k -column $k -row 3
    }
    ...
    foreach i {15 17 19} {
      incr k
      entry .syncvmlc$id.bottom.latch$k -textvariable cmval($id,$i,3) -width 20
      grid configure .syncvmlc$id.bottom.latch$k -column $k -row 4
    }
    ...
    foreach i {16 18 20} {
      ...
      menubutton .syncvmlc$id.bottom.med$k -text $cmval($id,$i) -menu \
            .syncvmlc$id.bottom.med$k.m -bd 1 -relief raised -indicatoron 1\
            -width 15
      menu .syncvmlc$id.bottom.med$k.m -tearoff no
      for {set iopt 0} {$iopt <= $nmed} {incr iopt} {
        .syncvmlc$id.bottom.med$k.m add command -label $medium($iopt) \
            -command "set_material .syncvmlc$id.bottom.med$k $iopt $id,$i" -columnbreak [expr $iopt % $med_per_column==0]
      }
      grid configure .syncvmlc$id.bottom.med$k -column $k -row 5
    }

    pack .syncvmlc$id.bottom -side top -pady 5

    frame .syncvmlc$id.buts -bd 4
    button .syncvmlc$id.buts.okb -text "OK" -command "destroy .syncvmlc$id"\
	    -relief groove -bd 8
    button .syncvmlc$id.buts.helpb -text "Help" -command\
	    "help_gif .syncvmlc$id.help {$help_syncvmlc_text} help_syncvmlc"\
	    -relief groove -bd 8
#don't have a preview yet, get it working after course, will call show_SYNCVMLC
    button .syncvmlc$id.buts.prev -text "Preview" -command\
                          "show_SYNCVMLC $id" -relief groove -bd 8
#	    { tk_dialog .nopreview "No Preview" \
#             "No preview available yet." info 0 OK } -relief groove -bd 8
    pack .syncvmlc$id.buts.helpb .syncvmlc$id.buts.okb .syncvmlc$id.buts.prev -side left -padx 10
    pack .syncvmlc$id.buts -pady 10
}

proc end_param_menu_syncvmlc { w id } {
    global cmval

    set inw .syncvmlc$id.top.endparam.inp18.label
...
proc define_syncvmlc_leaves { id } {
...
# leaf types
    ...
    catch { destroy .syncvmlc$id.childx }
    toplevel .syncvmlc$id.childx
    set top .syncvmlc$id.childx
  ...
  "help_gif .syncvmlc$id.fullhelp {$help_dynvmlc_full_text} help_dynvmlc_full"
    label $w.grid.full.t -text "FULL leaves"
    pack $w.grid.full.b $w.grid.full.t -side left

    frame $w.grid.target
    button $w.grid.target.b -text "?" -command\
  "help_gif .syncvmlc$id.tarhelp {$help_dynvmlc_tar_text} help_dynvmlc_tar"
  ...
  "help_gif .syncvmlc$id.isohelp {$help_dynvmlc_iso_text} help_dynvmlc_iso"
    ...
    button $top.cd.check -text "Check dimensions" -command \
      "check_dimensions_syncvmlc $id" -relief groove -bd 8
    button $top.cd.done -text "Done" -command\
            "destroy .syncvmlc$id.childx" -relief groove -bd 8
    pack $top.cd.check $top.cd.done -side left -padx 5
    pack $top.cd -pady 10 -side top

}

proc check_dimensions_syncvmlc { id } {
...
proc check_def_syncvmlc { id leafind } {
...
proc check_z_posn_syncvmlc { id leafind } {
    global cmval dimtext totprob
...
proc define_syncvmlc_types { id } {
    global cmval helvfont ngroups tow fromw help_syncvmlc_text
...
# number of leaves
    ...
    catch { destroy .syncvmlc$id.childt }
    toplevel .syncvmlc$id.childt
    set top .syncvmlc$id.childt
    ...
    pack $w
    frame $top.b
    button $top.b.addb -text "Add a row" -command "add_syncvmlc_row_type $id"\
            -relief groove -bd 8
    button $top.b.delb -text "Delete last row"\
            -command "del_syncvmlc_row_type $id" -relief groove -bd 8
    button $top.b.okb -text "OK"\
            -command "save_syncvmlc_type $id" -relief groove -bd 8
    button $top.b.helpb -text "Help" -command\
          "help_gif .syncvmlc$id.childt.help {$help_syncvmlc_text} help_syncvmlc"\
            -relief groove -bd 8
    pack $top.b.addb $top.b.delb $top.b.okb $top.b.helpb -side left -padx 10
    pack $top.b -pady 10
}

proc define_syncvmlc_openings { id } {
    global cmval helvfont ngroups nleaf to from help_syncvmlc_text
    ...
    catch { destroy .syncvmlc$id.child }
    toplevel .syncvmlc$id.child
    set top .syncvmlc$id.child
    ...
    message $w.message -text "Specify the minimum and maximum $orientation\
	    value (for straight, focused leaf ends, this is defined at ZMIN)\
            for the opening in \
	    each leaf in the SYNCVMLC by groups here.  Add a row to start a\
	    new group.  When a new row is added the next leaf in the order will\
    ...
    pack $w
    frame $top.b
    button $top.b.addb -text "Add a row" -command "add_syncvmlc_row $id"\
	    -relief groove -bd 8
    button $top.b.delb -text "Delete last row"\
            -command "del_syncvmlc_row $id" -relief groove -bd 8
    button $top.b.okb -text "OK" -command "save_syncvmlc $id" -relief groove -bd 8
    button $top.b.helpb -text "Help" -command\
	    "help_gif .syncvmlc$id.child.help {$help_syncvmlc_text} help_syncvmlc"\
	    -relief groove -bd 8
    pack $top.b.addb $top.b.delb $top.b.okb $top.b.helpb -side left -padx 10
    pack $top.b -pady 10
}

proc save_syncvmlc { id } {
    ...
    destroy .syncvmlc$id.child

}

proc save_syncvmlc_type { id } {
    ...
    destroy .syncvmlc$id.childt

}

proc add_syncvmlc_row { id } {
    global ngroups from to cmval nleaf

    set w .syncvmlc$id.child.f
...
proc add_syncvmlc_row_type { id } {
    global cmval fromw tow nleaf

    set w .syncvmlc$id.childt.f
...
proc show_SYNCVMLC { id } {
    global cmval helvfont medium nmed xrange yrange zrange med cm_ticks nleaf

    catch { destroy .syncvmlc$id.show }
    toplevel .syncvmlc$id.show
    wm title .syncvmlc$id.show "Preview"
    ...
    frame .syncvmlc$id.show.frm

    draw_SYNCVMLC $id

    frame .syncvmlc$id.show.buts
    button .syncvmlc$id.show.buts.range -text "Change plot properties" -command\
	    "change_cm_range $id .syncvmlc$id.show xyz" -relief groove -bd 8
    button .syncvmlc$id.show.buts.print1 -text "Print xy..." -command\
	    "print_canvas .syncvmlc$id.show.frm.can.one 450 450" -relief groove -bd 8
    button .syncvmlc$id.show.buts.print2 -text "Print xz..." -command\
	    "print_canvas .syncvmlc$id.show.frm.can.two 450 450" -relief groove -bd 8
    button .syncvmlc$id.show.buts.print3 -text "Print yz..." -command\
	    "print_canvas .syncvmlc$id.show.frm.can.three 450 450" -relief groove -bd 8
    button .syncvmlc$id.show.buts.done -text "Done" -command\
	    "destroy .syncvmlc$id.show" -relief groove -bd 8
    pack .syncvmlc$id.show.buts.range .syncvmlc$id.show.buts.print1\
	    .syncvmlc$id.show.buts.print2 .syncvmlc$id.show.buts.print3\
	    .syncvmlc$id.show.buts.done -side left -padx 10
    pack .syncvmlc$id.show.buts -side bottom -anchor s -pady 15
}

proc draw_SYNCVMLC { id } {
    global helvfont zrange yrange xrange zscale yscale xscale l m
    global cmval cm_ticks nleaf

    catch { destroy .syncvmlc$id.show.frm.can }
	...
	catch { destroy .syncvmlc$id.show.frm.scrlz }
	catch { destroy .syncvmlc$id.show.frm.scrlx }
	scrollbar .syncvmlc$id.show.frm.scrlz -command\
		".syncvmlc$id.show.frm.can yview"
	scrollbar .syncvmlc$id.show.frm.scrlx -command\
		".syncvmlc$id.show.frm.can xview" -orient horizontal
	pack .syncvmlc$id.show.frm.scrlz -side right -fill y
	pack .syncvmlc$id.show.frm.scrlx -side bottom -fill x
	canvas .syncvmlc$id.show.frm.can -width $winwidth -height $winheight\
		-yscrollcommand ".syncvmlc$id.show.frm.scrlz set"\
		-xscrollcommand ".syncvmlc$id.show.frm.scrlx set"\
		-scrollregion "0 0 $scrlwidth $scrlheight"
    } else {
	canvas .syncvmlc$id.show.frm.can -width $scrlwidth -height $scrlheight
    }

    # Put some text in the upper left corner, just to fill the gap.
    if {$cmval($id,2,2)==1 | $cmval($id,2,2)==2} {
      .syncvmlc$id.show.frm.can create text 225 225 -text "xz\
...
NOTE: This is a dynamic or step-and-shoot delivery, with leaf opening\
    ...
    canvas .syncvmlc$id.show.frm.can.one -width $canwidth -height $canwidth

    add_SYNCVMLC_xy $id $xscale $yscale $xrange(0) $yrange(0)\
	    $l $m .syncvmlc$id.show.frm.can.one

    set curx 0
    set cury 0
    label .syncvmlc$id.show.frm.can.one.xy -text \
	    [format "(%6.3f, %6.3f)" $curx $cury] -bg white\
	    -bd 5 -width 16 -font $helvfont
    .syncvmlc$id.show.frm.can.one create window [expr $l/2+$width]\
	    [expr 2*$m+$width] -window .syncvmlc$id.show.frm.can.one.xy
    bind .syncvmlc$id.show.frm.can.one <Motion> {
    ...
    coverup $l $m $width .syncvmlc$id.show.frm.can.one

    add_axes xy $cm_ticks($id,x) $cm_ticks($id,y) $l $m $width $xrange(0)\
	    $xrange(1) $yrange(0) $yrange(1)\
	    $xscale $yscale .syncvmlc$id.show.frm.can.one

    .syncvmlc$id.show.frm.can create window [expr $canwidth+$canwidth/2.0]\
	    [expr $canwidth/2.0] -window .syncvmlc$id.show.frm.can.one

    # XZ, lower left: if leaves // to y, show leaf sides,
    # else if leaves // to x, show only one leaf at y=0 (leaf ends).

    canvas .syncvmlc$id.show.frm.can.two -width $canwidth -height $canwidth

    add_air $id .syncvmlc$id.show.frm.can.two $xrange(0) $zrange(0)\
	    $xscale $zscale $l $m

    if $cmval($id,2,0)==0 {
	add_SYNCVMLC_sides $id $xscale $zscale $xrange(0) $zrange(0) $zrange(1)\
		$l $m .syncvmlc$id.show.frm.can.two
    } else {
        add_SYNCVMLC_ends $id $xscale $zscale $xrange(0) $zrange(0) $zrange(1)\
		$l $m .syncvmlc$id.show.frm.can.two
    }

    coverup $l $m $width .syncvmlc$id.show.frm.can.two

    label .syncvmlc$id.show.frm.can.two.xy -text \
	    [format "(%6.3f, %6.3f)" $curx $cury] -bg white\
	    -bd 5 -width 16 -font $helvfont
    .syncvmlc$id.show.frm.can.two create window [expr $l/2+$width]\
	    [expr 2*$m+$width] -window .syncvmlc$id.show.frm.can.two.xy
    bind .syncvmlc$id.show.frm.can.two <Motion> {
    ...
    add_axes xz $cm_ticks($id,x) $cm_ticks($id,z) $l $m $width $xrange(0)\
	    ...
	    $xscale $zscale .syncvmlc$id.show.frm.can.two

    .syncvmlc$id.show.frm.can create window [expr $canwidth/2.0]\
	    [expr $canwidth+$canwidth/2.0] -window .syncvmlc$id.show.frm.can.two

    # YZ, lower right

    canvas .syncvmlc$id.show.frm.can.three -width $canwidth -height $canwidth

    add_air $id .syncvmlc$id.show.frm.can.three $yrange(0) $zrange(0)\
	    $yscale $zscale $l $m

    if $cmval($id,2,0)==0 {
	add_SYNCVMLC_ends $id $yscale $zscale $yrange(0) $zrange(0) $zrange(1)\
		$l $m .syncvmlc$id.show.frm.can.three
    } else {
	add_SYNCVMLC_sides $id $yscale $zscale $yrange(0) $zrange(0) $zrange(1)\
		$l $m .syncvmlc$id.show.frm.can.three
    }

    coverup $l $m $width .syncvmlc$id.show.frm.can.three

    label .syncvmlc$id.show.frm.can.three.xy -text \
	    [format "(%6.3f, %6.3f)" $curx $cury] -bg white\
	    -bd 5 -width 16 -font $helvfont
    .syncvmlc$id.show.frm.can.three create window [expr $l/2+$width]\
	    [expr 2*$m+$width] -window .syncvmlc$id.show.frm.can.three.xy
    bind .syncvmlc$id.show.frm.can.three <Motion> {
    ...
    add_axes yz $cm_ticks($id,y) $cm_ticks($id,z) $l $m $width $yrange(0)\
	    ...
	    $yscale $zscale .syncvmlc$id.show.frm.can.three

    .syncvmlc$id.show.frm.can create window [expr $canwidth+$canwidth/2.0]\
	    [expr $canwidth+$canwidth/2.0] -window .syncvmlc$id.show.frm.can.three

    pack .syncvmlc$id.show.frm.can -side top -anchor n
    pack .syncvmlc$id.show.frm -side top -anchor n
}

proc add_SYNCVMLC_xy {id xscale yscale xmin ymin l m parent_w} {
...
proc add_SYNCVMLC_ends {id xscale zscale xmin zmin zmax l m parent_w} {
...
proc add_SYNCVMLC_sides {id yscale zscale ymin zmin zmax l m parent_w} {
...
proc help_ignoregaps_syncvmlc { w } {

# help procedure put here because it is only for SYNCVMLC and VARMLC
...
in SYNCVMLC by a factor of 2, while preserving exact transport in \
the leaf ends.\
...
# help procedure put here because it is only for SYNCVMLC
...
of the leaf opening for field I.  NUM instructs SYNCVMLC to apply the\
accompanying values of NEG and POS to NUM adjacent leaves. NUM defaults\
to 1.  NEG POS NUM inputs start with leaf no. 1.\
See the general SYNCVMLC help for leaf numbering scheme.
