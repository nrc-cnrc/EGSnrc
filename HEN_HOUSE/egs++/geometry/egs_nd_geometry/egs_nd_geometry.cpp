...
EGS_NDGeometry::EGS_NDGeometry(int ng, EGS_BaseGeometry **G,
    ...
    nreg = 1; setup();
    for(int j=0; j<N; j++) {
        g[j] = G[j]; n[j] = nreg; nreg *= g[j]->regions();
        if( !g[j]->isConvex() ) is_convex = false;
    }
    n[N] = nreg;
}
...
EGS_NDGeometry::~EGS_NDGeometry() {
    for(int j=0; j<N; j++) {
        int ref = g[j]->deref(); if( !ref ) delete g[j];
    }
    delete [] n; delete [] g;
}

void EGS_NDGeometry::setup() {
    n = new int [N+1]; g = new EGS_BaseGeometry* [N];
}
...
void EGS_NDGeometry::setMedia(EGS_Input *input, int nmed, const int *mind) {
    ...
    while( (i = input->takeInputItem("set medium")) ) {
        vector<int> inp;
        int err = i->getInput("set medium",inp); delete i;
        if( !err ) {
            if( inp.size() == 2 ) setMedium(inp[0],inp[0],mind[inp[1]]);
                    ...
                    inp[3]);
            else if( inp.size() == 2*N+1 ) setM(0,0,inp,inp[2*N]);
...
EGS_XYZGeometry::EGS_XYZGeometry(EGS_PlanesX *Xp, EGS_PlanesY *Yp,
        ...
        xp(Xp), yp(Yp), zp(Zp) {
    nx = xp->regions(); ny = yp->regions(); nz = zp->regions();
    nxy = nx*ny; nreg = nxy*nz;
    xp->ref(); yp->ref(); zp->ref();
...
EGS_XYZGeometry::~EGS_XYZGeometry() {
    if( !xp->deref() ) delete xp;
    if( !yp->deref() ) delete yp;
    if( !zp->deref() ) delete zp;
}

void EGS_XYZGeometry::printInfo() const {
    EGS_BaseGeometry::printInfo();
    xp->printInfo(); yp->printInfo(); zp->printInfo();
    egsInformation(
            "=======================================================\n");
}
    ...
    while( (i = input->takeInputItem("set medium")) ) {
        ...
        if( !err ) {
            if( inp.size() == 2 ) setMedium(inp[0],inp[0],mind[inp[1]]);
                ...
                if( is < 0 ) is = 0; if( ie > nx-1 ) ie = nx-1;
                int js = inp[2], je = inp[3];
                if( js < 0 ) js = 0; if( je > nxy/nx-1 ) je = nxy/nx-1;
                int ks = inp[4], ke = inp[5];
                if( ks < 0 ) ks = 0; if( ke > nreg/nxy-1 ) ke = nreg/nxy-1;
                ...
                if( js < 0 ) js = 0; if( je > nxy/nx-1 ) je = nxy/nx-1;
                if( jdelta <= 0 ) jdelta = 1;
                int ks = inp[6], ke = inp[7], kdelta = inp[8];
                if( ks < 0 ) ks = 0; if( ke > nreg/nxy-1 ) ke = nreg/nxy-1;
                if( kdelta <= 0 ) kdelta = 1;
                //egsInformation("setting medium to %d between %d %d %d %d "
                //        "%d %d\n",mind[inp[6]],is,ie,js,je,ks,ke);
...
EGS_XYZGeometry* EGS_XYZGeometry::constructGeometry(const char *dens_file,
    ...
    if( dens_or_egsphant_or_interfile == 2 ) {
        ...
        egsInformation("Using medium %s for rho=%g...%g\n",medname.c_str(),
                rmin,rmax);
        med_names.push_back(medname);
        rho_min.push_back(rmin); rho_max.push_back(rmax);
        rho_def.push_back(rdef);
    }
    if( med_names.size() < 1 ) {
    ...
    int Nx, Ny, Nz; EGS_Float *xx, *yy, *zz; float *rho;
    if( dens_or_egsphant_or_interfile == 0 ) {
        ...
        dens.read((char *) &Nx,sizeof(int)); if( swap ) egsSwapBytes(&Nx);
        dens.read((char *) &Ny,sizeof(int)); if( swap ) egsSwapBytes(&Ny);
        dens.read((char *) &Nz,sizeof(int)); if( swap ) egsSwapBytes(&Nz);
        if( Nx < 1 || Ny < 1 || Nz < 1 ) {
            egsWarning("%s: invalid density matrix file: "
                    " Nx=%d Ny=%d Nz=%d\n",func,Nx,Ny,Nz); return 0;
        }
        ...
        if( swap ) {
            int j;
            for(j=0; j<Nx; j++) egsSwapBytes(&x[j]);
            for(j=0; j<Ny; j++) egsSwapBytes(&y[j]);
            for(j=0; j<Nz; j++) egsSwapBytes(&z[j]);
        }
        {
            int j;
            xx = new EGS_Float [Nx+1]; for(j=0; j<=Nx; j++) xx[j] = x[j];
            yy = new EGS_Float [Ny+1]; for(j=0; j<=Ny; j++) yy[j] = y[j];
            zz = new EGS_Float [Nz+1]; for(j=0; j<=Nz; j++) zz[j] = z[j];
        ...
        if( dens.fail() ) {
            ...
            delete [] rho; delete [] xx; delete [] yy; delete [] zz; return 0;
        }
        if( swap ) {
            for(int j=0; j<Nx*Ny*Nz; j++) egsSwapBytes(&rho[j]);
        }
    }
        ...
        while(1) {
            string line, key, value; size_t pos;
            getline(h_file, line);
            if( h_file.eof() || h_file.fail() || !h_file.good() ) break;

            pos = line.find(":=");
            if( pos == string::npos ) continue;
            key = line.substr(0, int(pos));
            value = line.substr(int(pos)+2);
            while ((key[0] == '!') || (key[0] == ' ')) key.erase(0,1);
            while (key[key.length()-1] == ' ') key.erase(key.length()-1, 1);
            while (value[0] == ' ') value.erase(0,1);
            while (value[value.length()-1] == ' ') value.erase(value.length()-1, 1);

            if ( key ==  "matrix size [1]" ) { sscanf(value.c_str(), "%u", &Nx);  }
            else if ( key ==  "matrix size [2]" ) { sscanf(value.c_str(), "%u", &Ny);  }
            else if ( ( key ==  "number of slices" ) || (key ==  "number of images") ) { sscanf(value.c_str(), "%u", &Nz); }
            else if ( key ==  "scaling factor (mm/pixel) [1]" ) { sscanf(value.c_str(), "%f", &scale_x); }
            else if ( key ==  "scaling factor (mm/pixel) [2]" ) { sscanf(value.c_str(), "%f", &scale_y); }
            else if ( key ==  "slice thickness (pixels)" ) { sscanf(value.c_str(), "%f", &scale_z); } // ????????????????
            else if ( key ==  "name of data file" ) { data_file = value; }
            else if ( key ==  "number format" ) {
                if( (value == "float") || (value == "FLOAT") ) data_type = 0;
                else if( (value == "unsigned integer") || (value == "UNSIGNED INTEGER") ) data_type = 1;
                else egsWarning("%s: unrecognised 'number format' type: %s \n",func,value.c_str());
            }
        }
        if( Nx < 1 || Ny < 1 || Nz < 1 || scale_x <= 0.0 || scale_y <= 0.0 || scale_z <= 0.0 || data_file == "" || data_type == -1) {
            egsWarning("%s: invalid interfile header information: "
                    ...
                    "data_file='%s' number_format=%d\n",func,Nx,Ny,Nz,scale_x,scale_y,scale_z,data_file.c_str(),data_type); return 0;
        }
        ifstream i_file(data_file.c_str(),ios::binary);
        if( !i_file ) {
            egsWarning("%s: failed to open interfile data "
                    "%s\n",func,data_file.c_str()); return 0;
        }
        scale_x /= 10.0; scale_y /= 10.0; scale_z /= 10.0; // convert to cm
        {
            int j;
            xx = new EGS_Float [Nx+1]; for(j=0; j<=Nx; j++) xx[j] = -(float)Nx*scale_x/2.0 + j*scale_x;
            yy = new EGS_Float [Ny+1]; for(j=0; j<=Ny; j++) yy[j] = -(float)Ny*scale_y/2.0 + j*scale_y;
            zz = new EGS_Float [Nz+1]; for(j=0; j<=Nz; j++) zz[j] = -(float)Nz*scale_z/2.0 + j*scale_z;
        }
        rho = new float [Nx*Ny*Nz];
        if (data_type == 0) {
            ...
            for (int cc = 0; cc<Nx*Ny*Nz; cc++) rho[cc] = (float) (rho_tmp[cc]);
            delete [] rho_tmp;
        }
        if( i_file.fail() ) {
        ...
        // ignore estepe
        EGS_Float dum; for(imed=0; imed<nmed; ++imed) data >> dum;
        data >> Nx >> Ny >> Nz;
        if( data.fail() ) {
            egsWarning("%s: failed reading number of voxels\n",func); return 0;
        }
        xx = new EGS_Float [Nx+1]; yy = new EGS_Float [Ny+1]; zz = new EGS_Float [Nz+1];
        int j;
        for(j=0; j<=Nx; ++j) data >> xx[j];
        if( data.fail() ) {
            egsWarning("%s: failed reading x-planes\n",func);
            delete [] xx; delete [] yy; delete [] zz; return 0;
        }
        for(j=0; j<=Ny; ++j) data >> yy[j];
        if( data.fail() ) {
            egsWarning("%s: failed reading y-planes\n",func);
            delete [] xx; delete [] yy; delete [] zz; return 0;
        }
        for(j=0; j<=Nz; ++j) data >> zz[j];
        if( data.fail() ) {
            egsWarning("%s: failed reading z-planes\n",func);
            delete [] xx; delete [] yy; delete [] zz; return 0;
        }
        int nr = Nx*Ny*Nz; int med;
        //for(j=0; j<nr; ++j) data >> med;
        data.getline(buf,1023);
        for(int iz=0; iz<Nz; ++iz) {
            for(int iy=0; iy<Ny; ++iy) data.getline(buf,1023);
            data.getline(buf,1023);
        }
        if( data.fail() ) {
    ...
    EGS_Float rhomin=1e30,rhomax=0; int j;
    for(j=0; j<Nx*Ny*Nz; j++) {
        if( rho[j] > rhomax ) rhomax = rho[j];
        if( rho[j] < rhomin ) rhomin = rho[j];
    }
    egsInformation("Min. density found: %g\n",rhomin);
    egsInformation("Max. density found: %g\n",rhomax);
    int nmed = med_names.size();
    int *imed = new int [nmed];
    for(j=0; j<nmed; j++) imed[j] = -2;
    for(j=0; j<Nx*Ny*Nz; j++) {
        int i;
        for(i=0; i<nmed; i++) {
            if( rho[j] >= rho_min[i] && rho[j] < rho_max[i] ) break;
        }
        if( i >= nmed ) {
...
int EGS_DeformedXYZ::setDeformations(const char *defFile) {
    ...
    if( !vectors ) vectors = new EGS_Vector [nvec];
    float tmp[3]; int i,j,k;
    for(j=0; j<nvec; ++j)  {
    ...
    for(j=0; j<24; ++j) {
        int i = tetrahedra[j];
        switch (i) {
            case 0: tnodes[j] = 0; break;
            case 1: tnodes[j] = 1; break;
            case 2: tnodes[j] = nx+1; break;
            case 3: tnodes[j] = nx+2; break;
            case 4: tnodes[j] = (nx+1)*(ny+1); break;
            case 5: tnodes[j] = (nx+1)*(ny+1)+1; break;
            case 6: tnodes[j] = (nx+1)*(ny+1)+nx+1; break;
            case 7: tnodes[j] = (nx+1)*(ny+1)+nx+2; break;
        }
    }
...
EGS_XYZRepeater::EGS_XYZRepeater(EGS_Float xmin, EGS_Float xmax,
    ...
    dx = (xmax - xmin)/nx; dxi = 1/dx; ax = -xmin*dxi;
    dy = (ymax - ymin)/ny; dyi = 1/dy; ay = -ymin*dyi;
    dz = (zmax - zmin)/nz; dzi = 1/dz; az = -zmin*dzi;
    EGS_PlanesX *xp; EGS_PlanesY *yp; EGS_PlanesZ *zp;
    ...
    xyz->ref(); g->ref();
    nxy = nx*ny; nxyz = nx*ny*nz; ng = g->regions();
    nreg = nxyz*ng + 1;
...
void EGS_XYZRepeater::printInfo() const {
    ...
    else  egsInformation("box is filled with vacuum\n");
...
void EGS_XYZGeometry::voxelizeGeometry(EGS_Input *input) {
    ...
    if( rhor ) { delete [] rhor; rhor = 0; }
    region_media = new short [nreg];
    bool hrs = geometry->hasRhoScaling();
    if( hrs ) {
        has_rho_scaling = true; rhor = new EGS_Float [nreg];
    }
    else has_rho_scaling = false;
    ...
    vector<string> options; options.push_back("no"); options.push_back("yes");
...
EGS_NDG_EXPORT EGS_BaseGeometry* createGeometry(EGS_Input *input) {
    ...
    if( !is_xyz && input->compare("EGS_XYZRepeater",type) ) {
        string base,medium; vector<EGS_Float> xr, yr, zr;
        ...
        if( err1 ) return 0;
        EGS_Float xmin = xr[0], xmax = xr[1]; int nx = (int) (xr[2]+0.1);
        if( xmin >= xmax || nx < 1 ) {
            egsWarning("%s: wrong 'repeat x' input xmin=%g xmax=%g nx=%d\n",
                    xmin,xmax,nx); err1 = 1;
        }
        EGS_Float ymin = yr[0], ymax = yr[1]; int ny = (int) (yr[2]+0.1);
        if( ymin >= ymax || ny < 1 ) {
            egsWarning("%s: wrong 'repeat y' input ymin=%g ymax=%g ny=%d\n",
                    ymin,ymax,ny); err1 = 1;
        }
        EGS_Float zmin = zr[0], zmax = zr[1]; int nz = (int) (zr[2]+0.1);
        if( zmin >= zmax || nz < 1 ) {
        ...
        if( !err5 ) result->setMedium(medium);
        ...
        if( !ierr1 ) dens_or_egsphant_or_interfile = 0;
        else if( !ierr3 ) { dens_or_egsphant_or_interfile = 1; dens_file = egsphant_file; }
        else if( !ierr4 ) { dens_or_egsphant_or_interfile = 2; dens_file = interfile_file; }
        if( dens_or_egsphant_or_interfile >= 0 || !ierr2 ) {
            ...
            result->setName(input); return result;
        }
    ...
    int n_concav = 0;
    for(int j=0; j<dims.size(); j++) if( !dims[j]->isConvex() ) n_concav++;
    bool is_ok = true;
    if( n_concav > 1 ) {
    ...
    if( !is_ok ) {
        for(int j=0; j<dims.size(); j++)
            if( dims[j]->deref() ) delete dims[j];
        return 0;
    }
    int hn_method=0; err1 = input->getInput("hownear method",hn_method);
    EGS_BaseGeometry *result;
    if( !err1 && hn_method == 1 ) result = new EGS_NDGeometry(dims,"",false);
    else result = new EGS_NDGeometry(dims);
    result->setName(input);
...
void EGS_XYZGeometry::setXYZLabels (EGS_Input *input) {
    ...
    if (!err) xp->setLabels(inp);

    err = input->getInput("set y label", inp);
    if (!err) yp->setLabels(inp);

    err = input->getInput("set z label", inp);
    if (!err) zp->setLabels(inp);
}
...
int EGS_NDGeometry::ndRegions (int r, int dim, int dimk, int k, vector<int> &regs) {
    ...
    if (dim == dimk) {
        r += k*n[dimk];
        if (dim == N-1) regs.push_back(r);
        else ndRegions(r, dim+1, dimk, k, regs);
    }
...
void EGS_NDGeometry::getLabelRegions (const string &str, vector<int> &regs) {
    ...
    for (int i=0; i<N; i++) {
        local_regs.clear();
        if (g[i]) g[i]->getLabelRegions(str, local_regs);
        if (local_regs.size() == 0) continue;
