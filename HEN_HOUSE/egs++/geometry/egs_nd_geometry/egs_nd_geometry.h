...
public:
      ...
      */
      ...
      bool isInside(const EGS_Vector &x) {
          for(int j=0; j<N; j++) if( !g[j]->isInside(x) ) return false;
          return true;
      };
      ...
      EGS_Float howfarToOutside(int ireg, const EGS_Vector &x,
              const EGS_Vector &u) {
          if( ireg < 0 ) return 0;
          int itmp = ireg; EGS_Float d = 1e30;
          for(int j=N-1; j>=0; j--) {
              int l = itmp/n[j];
      ...
      int howfar(int ireg, const EGS_Vector &x, const EGS_Vector &u,
          ...
          if( ireg >= 0 ) {
              int itmp = ireg; int inext = -1, idelta, lnew_j;
              for(int j=N-1; j>=0; j--) {
                  int l = itmp/n[j];
                  int lnew = g[j]->howfar(l,x,u,t,0,normal);
                  if( lnew != l ) {
                      inext = j; idelta = lnew - l; lnew_j = lnew;
                  }
                  itmp -= l*n[j];
              }
              if( inext < 0 ) return ireg;
          ...
          for(int j=0; j<N; j++) {
                  ...
                  if( i >= 0 ) {
                      ...
                      for(int k=0; k<N; k++) {
                          if( k != j ) {
                              ...
                              if( check < 0 ) { is_ok = false; break; }
                              res += check*n[k];
                          }
                      }
                      if( is_ok ) {
                          t = tj; res += i*n[j];
                          if( newmed ) *newmed = medium(res);
                          if( normal ) *normal = nn;
                          return res;
                      }
                  }
              ...
              // dimension is last in the list.
                  ...
                  EGS_Vector tmp(x); EGS_Float tleft = t;
                  ...
                  while(1) {
                      ...
                      if( inew == ii ) break;
                      tleft -= tt; tmp += u*tt; ii = inew; ttot += tt;
                  ...
                  if( ii < 0 ) {
                      ...
                      if( inew >= 0 ) {
                          tmp += u*tt; bool is_ok = true; int res = 0;
                          for(int k=0; k<N; k++) {
                              if( k != j ) {
                                  int check = g[k]->isWhere(tmp);
                                  if( check < 0 ) { is_ok = false; break; }
      ...
      EGS_Float hownear(int ireg, const EGS_Vector &x) {
          ...
          if( ortho ) {
              ...
              EGS_Float tmin = 1e30;
              for(int j=0; j<N; j++) {
                  EGS_Float t; int i = g[j]->isWhere(x);
                  t = g[j]->hownear(i,x);
                  if( t < tmin ) { tmin = t; if( tmin <= 0 ) return 0; }
              }
              return tmin;
          }
      };
      ...
      int getMaxStep() const {
          int nstep = 1;
          for(int j=0; j<N; ++j) nstep += g[j]->getMaxStep();
          return nstep;
      };
...
public:
      ...
      bool isInside(const EGS_Vector &x) {
          if( !xp->isInside(x) ) return false;
          if( !yp->isInside(x) ) return false;
          if( !zp->isInside(x) ) return false;
          return true;
      };

      int isWhere(const EGS_Vector &x) {
          int ix = xp->isWhere(x); if( ix < 0 ) return ix;
          int iy = yp->isWhere(x); if( iy < 0 ) return iy;
          int iz = zp->isWhere(x); if( iz < 0 ) return iz;
      ...
      int computeIntersections(int ireg, int n, const EGS_Vector &X,
          ...
          if( u.x > 0 )      { uxi = 1/u.x; dirx =  1; icx = 1; }
          else if( u.x < 0 ) { uxi = 1/u.x; dirx = -1; icx = 0; }
          else               { uxi = 1e33;  dirx =  1; icx = 1; }
          if( u.y > 0 )      { uyi = 1/u.y; diry =  1; icy = 1; }
          else if( u.y < 0 ) { uyi = 1/u.y; diry = -1; icy = 0; }
          else               { uyi = 1e33;  diry =  1; icy = 1; }
          if( u.z > 0 )      { uzi = 1/u.z; dirz =  1; icz = 1; }
          else if( u.z < 0 ) { uzi = 1/u.z; dirz = -1; icz = 0; }
          else               { uzi = 1e33;  dirz =  1; icz = 1; }
          nextx = (px[ix+icx] - x.x)*uxi + t_ini;
          nexty = (py[iy+icy] - x.y)*uyi + t_ini;
          nextz = (pz[iz+icz] - x.z)*uzi + t_ini;
          for(int j=ifirst; j<n; j++) {
              isections[j].imed = imed; isections[j].ireg = ireg;
              ...
              if( nextx < nexty && nextx < nextz ) {
                  ...
                  if( ix < 0 || ix >= nx ) inew = -1;
                      ...
                      inew = ireg + dirx; nextx = (px[ix+icx] - x.x)*uxi + t_ini;
                  }
              }
              else if( nexty < nextz ) {
                  t = nexty; iy += diry;
                  if( iy < 0 || iy >= ny ) inew = -1;
                      ...
                      inew = ireg + nx*diry; nexty = (py[iy+icy] - x.y)*uyi + t_ini;
                  }
              }
              else {
                  t = nextz; iz += dirz;
                  if( iz < 0 || iz >= nz ) inew = -1;
                      ...
                      inew = ireg + nxy*dirz; nextz = (pz[iz+icz] - x.z)*uzi + t_ini;
                  }
              }
              isections[j].t = t;
              if( inew < 0 ) return j+1;
              ireg = inew; imed = medium(ireg);
          }
          return ireg >= 0 ? -1 : n;
      };
      ...
      EGS_Float howfarToOutside(int ireg, const EGS_Vector &x,
          ...
          EGS_Float ty = yp->howfarToOutside(iy,x,u); if( ty < d ) d = ty;
          EGS_Float tz = zp->howfarToOutside(iz,x,u); if( tz < d ) d = tz;
          return d;
      };
      ...
      int howfar(int ireg, const EGS_Vector &x, const EGS_Vector &u,
          ...
          if( ireg >= 0 ) {
              ...
              if( newmed && inew >= 0 ) *newmed = medium(inew);
              return inew;
          }
          int face,ix,iy,iz;
          int inew = howfarFromOut(x,u,t,ix,iy,iz,face,normal);
          if( inew >= 0 && newmed ) *newmed = medium(inew);
          return inew;
      };
     ...
     EGS_Float getBound(int idir, int ind) {
          EGS_Float bound;
          if(idir == 0) {
            if(ind>=0 && ind<=nx) bound = xpos[ind];
            else {
            ...
            if(ind>=0 && ind<=ny) bound = ypos[ind];
            else {
            ...
            if(ind>=0 && ind<=nz) bound = zpos[ind];
            else {
      ...
      int getNRegDir(int idir) {
          if(idir==0) return nx;
          else if(idir==1) return ny;
          else if(idir==2) return nz;
          else {
      ...
      int getMaxStep() const { return nx+ny+nz+1; };

      int getNx() const { return nx; };
      int getNy() const { return ny; };
      int getNz() const { return nz; };

      EGS_Float *getXPositions() { return xp->getPositions(); };
      EGS_Float *getYPositions() { return yp->getPositions(); };
      EGS_Float *getZPositions() { return zp->getPositions(); };
      ...
      virtual void getLabelRegions (const string &str, vector<int> &regs);
      void getXLabelRegions (const string &str, vector<int> &regs) { xp->getLabelRegions(str, regs); }
      void getYLabelRegions (const string &str, vector<int> &regs) { yp->getLabelRegions(str, regs); }
      void getZLabelRegions (const string &str, vector<int> &regs) { zp->getLabelRegions(str, regs); }
...
protected:
      ...
      int howfarFromOut(const EGS_Vector &x, const EGS_Vector &u,
          ...
          ix = -1; EGS_Float t1;
          if( x.x <= xpos[0] && u.x > 0 ) {
              t1 = (xpos[0]-x.x)/u.x; ix = 0; face = 0;
          }
          else if( x.x >= xpos[nx] && u.x < 0 ) {
              t1 = (xpos[nx]-x.x)/u.x; ix = nx-1; face = 1;
          }
          if( ix >= 0 && t1 <= t ) {
              ...
              if( iy >= 0 ) {
                  ...
                  if( iz >= 0 ) {
                      ...
                      if (t1 < epsilon) {
                          ...
                          if (itest == -1 && t1 < epsilon) return -1;
                      }
          ...
          iy = -1;
          if( x.y <= ypos[0] && u.y > 0 ) {
              t1 = (ypos[0]-x.y)/u.y; iy = 0; face = 2;
          }
          else if( x.y >= ypos[ny] && u.y < 0 ) {
              t1 = (ypos[ny]-x.y)/u.y; iy = ny-1; face = 3;
          }
          if( iy >= 0 && t1 <= t ) {
              EGS_Vector tmp(x + u*t1);
              ix = xp->isWhere(tmp);
          ...
          iz = -1;
          if( x.z <= zpos[0] && u.z > 0 ) {
              t1 = (zpos[0]-x.z)/u.z; iz = 0; face = 4;
          }
          else if( x.z >= zpos[nz] && u.z < 0 ) {
              t1 = (zpos[nz]-x.z)/u.z; iz = nz-1; face = 5;
          }
          if( iz >= 0 && t1 <= t ) {
              EGS_Vector tmp(x + u*t1);
              ix = xp->isWhere(tmp);
...
public:
    ...
    int medium(int ireg) const { return EGS_XYZGeometry::medium(ireg/6); };
    ...
    int howfar(int ireg, const EGS_Vector &x, const EGS_Vector &u,
        ...
        if( ireg >= 0 ) {
            ...
            int ind[3];
            ind[2] = ir/nxy; ind[1] = (ir - ind[2]*nxy)/nx;
            ...
            EGS_Vector n0x(n0-x);
            EGS_Float disti[4],dpi[4]; EGS_Vector normvec;
            normvec = n1%n2; dpi[0]=normvec*u; disti[0]=normvec*n0x;
            normvec = n3%n1; dpi[2]=normvec*u; disti[2]=normvec*n0x;
            normvec = n2%n3; dpi[1]=normvec*u; disti[1]=normvec*n0x;
            normvec = (n3-n1)%(n2-n1);
            dpi[3] = normvec*u; disti[3] = normvec*(n0x+n1);
            int nextplane = 4; EGS_Float mindist = t, mindp=1;
            for(int i=0; i<4; ++i) {
                if( dpi[i] > 0 && disti[i]*mindp < mindist*dpi[i] ) {
                    mindist = disti[i]; mindp=dpi[i]; nextplane=i;
                }
            }
            if( nextplane == 4 ) return ireg;
            t = mindist/mindp;
            ...
            if( next >= 0 ) {
                ...
                if( ind[next] < 0 || ind[next] >= np[next] ) return -1;
                irnew = ind[0] + ind[1]*nx + ind[2]*nxy;
            }
            if( newmed ) *newmed = EGS_XYZGeometry::medium(irnew);
            if( normal ) {
                switch(nextplane) {
                    case 0: *normal = n2%n1; break;
                    case 1: *normal = n1%n3; break;
                    case 2: *normal = n3%n2; break;
                    case 3: *normal = (n2-n1)%(n3-n1);
                }
            }
        ...
        int ix,iy,iz,face;
        int inew = howfarFromOut(x,u,t,ix,iy,iz,face,normal);
        if( inew < 0 ) return inew;
        if( newmed ) *newmed = EGS_XYZGeometry::medium(inew);
        ...
        if( inTriangle(vectors[edge+node1],vectors[edge+node2],
                       vectors[edge+node3],tmp) ) return 6*inew + tetra1;
        ...
        if( inTriangle(vectors[edge+node1a],vectors[edge+node2a],
                       vectors[edge+node3a],tmp) ) return 6*inew + tetra2;
        // roundoff problem
    ...
    EGS_Float hownear(int ireg, const EGS_Vector &x) {
        if( ireg < 0 ) return EGS_XYZGeometry::hownear(ireg,x);
        int ir = ireg/6, tetra = ireg - 6*ir, tetra4 = 4*tetra;
        int ind[3];
        ind[2] = ir/nxy; ind[1] = (ir - ind[2]*nxy)/nx;
        ind[0] = ir - ind[2]*nxy - ind[1]*nx;
        ...
        EGS_Vector n0x(n0-x);
        EGS_Float disti[4],dpi[4]; EGS_Vector normvec;
        normvec = n1%n2; dpi[0]=normvec.length2(); disti[0]=normvec*n0x;
        normvec = n3%n1; dpi[2]=normvec.length2(); disti[2]=normvec*n0x;
        normvec = n2%n3; dpi[1]=normvec.length2(); disti[1]=normvec*n0x;
        normvec = (n3-n1)%(n2-n1);
        dpi[3] = normvec.length2(); disti[3] = normvec*(n0x+n1);
        EGS_Float mindist = 1e30, mindp=1;
        for(int i=0; i<4; ++i) {
            if( disti[i]*mindp < mindist*dpi[i] ) {
                mindist = disti[i]; mindp=dpi[i];
            }
        }
    ...
    EGS_Float howfarToOutside(int ireg, const EGS_Vector &x,
        ...
        if( ireg < 0 ) return 0;
        int ir = ireg/6;
        int iz = ir/nxy; int ir1 = ir - iz*nxy;
        int iy = ir1/nx; int ix = ir1 - iy*nx;
        EGS_Float  d = xp->howfarToOutside(ix,x,u);
        EGS_Float ty = yp->howfarToOutside(iy,x,u); if( ty < d ) d = ty;
        EGS_Float tz = zp->howfarToOutside(iz,x,u); if( tz < d ) d = tz;
        return d;
    ...
    int getMaxStep() const { return 6*(nx+ny+nz) + 1; };

    const string &getType() const { return def_type; };
    ...
    bool inTriangle(const EGS_Vector &n0, const EGS_Vector &n1,
        ...
        EGS_Float D=a*c-b*b;
        EGS_Float u=b1*c-b2*b; if(u<0||u>D) return false;
        EGS_Float v=b2*a-b1*b; if(v<0||u+v>D) return false;
        return true;
    };
...
public:
    ...
    bool isInside(const EGS_Vector &x) { return xyz->isInside(x); };

    int inside(const EGS_Vector &x) { return isWhere(x); };

    int medium(int ireg) const {
        if( ireg == nreg-1 ) return med;
        int cell = ireg/ng; int ilocal = ireg - ng*cell;
    ...
    int howfar(int ireg, const EGS_Vector &x, const EGS_Vector &u,
        ...
        int ix = (int) (x.x*dxi + ax); if( ix >= nx ) ix = nx-1;
        int iy = (int) (x.y*dyi + ay); if( iy >= ny ) iy = ny-1;
        int iz = (int) (x.z*dzi + az); if( iz >= nz ) iz = nz-1;
        ...
        while(1) {
            ...
            if( inew >= 0 ) {
                t = ttot + this_t; return cell*ng + inew;
            }
            int next_cell = xyz->howfar(cell,xtmp,u,this_t,0,normal);
            //egsWarning("next: %d %g\n",next_cell,this_t);
            if( next_cell == cell ) return ireg;
            if( next_cell < 0 ) {
                t = ttot + this_t; return -1;
            }
    ...
    EGS_Float hownear(int ireg, const EGS_Vector &x) {
        if( ireg < 0 ) return xyz->hownear(ireg,x);
        if( ireg < nreg-1 ) {
            int cell = ireg/ng; int ilocal = ireg - cell*ng;
            EGS_Vector xp(x - translation[cell]);
    ...
    void setXYZLabels(EGS_Input *input) {xyz->setXYZLabels(input); }
