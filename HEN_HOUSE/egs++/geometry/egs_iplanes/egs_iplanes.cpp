...
EGS_IPlanes::EGS_IPlanes(const EGS_Vector &Xo, const EGS_Vector &A, int np,
    ...
    nreg = 2*np;
    a = new EGS_Vector[nreg]; d = new EGS_Float[nreg]; int j;
    EGS_Float phi180 = M_PI;
    if( degree ) phi180 = 180;
    EGS_RotationMatrix R(axis);
    for(j=0; j<nreg; j++) {
        EGS_Float phi;
        if( j < np ) phi = angles[j]; else phi = angles[j-np] + phi180;
        if( degree ) phi *= (M_PI/180);
...
EGS_IPlanes::EGS_IPlanes(const EGS_Vector &Xo, const EGS_Vector &A,
    ...
    nreg = np;
    a = new EGS_Vector[nreg]; d = new EGS_Float[nreg]; int j;
    for(j=0; j<nreg; j++) { a[j] = aj[j]; d[j] = dj[j]; }
}
...
EGS_IPlanes::EGS_IPlanes(const EGS_Vector &Xo, const EGS_Vector &A,
    ...
    EGS_RotationMatrix R(axis);
    a = new EGS_Vector[nreg]; d = new EGS_Float[nreg];
...
int EGS_IPlanes::howfar(int ireg, const EGS_Vector &x,
    ...
    if( up < 0 ) {
        ...
        if( t1 <= t ) {
            t = t1; inew = ireg-1;
            if( inew < 0 ) inew = nreg-1;
            if( newmed ) *newmed = medium(inew);
            if( normal ) *normal = a[ireg];
        }
    }
    int j = ireg+1;
    if( j >= nreg ) j = 0;
...
EGS_RadialRepeater::EGS_RadialRepeater(const EGS_Vector &Xo,
    ...
    g->ref(); iplanes->ref();
    nrep = np; ng = g->regions(); nreg = nrep*ng + 1;
    //EGS_Float dphi = 2*M_PI/np;
...
EGS_RadialRepeater::~EGS_RadialRepeater() {
    delete [] R;
    if( !iplanes->deref() ) delete iplanes;
    if( !g->deref() ) delete g;
}
...
void EGS_RadialRepeater::printInfo() const {
    ...
    else  egsInformation("space is filled with vacuum\n");
...
EGS_IPLANES_EXPORT EGS_BaseGeometry* createGeometry(EGS_Input *input) {
    ...
    if( !err && (type == "EGS_RadialRepeater" || type == "repeater") ) {
        int np; err = input->getInput("number of repetitions",np);
        if( err || np < 2 ) {
    ...
    bool is_degree = true; EGS_Float max_angle = 180;
    if( err ) {
        ...
        is_degree = false; max_angle = M_PI;
    }
