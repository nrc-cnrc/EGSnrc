...
struct EGS_VHP_LOCAL VHP_RowInfo {
    ...
    unsigned char  *org;
    VHP_RowInfo() : nbin(0) {};
    ~VHP_RowInfo() { if( nbin > 0 ) { delete [] pix; delete [] org; } };
    int getBin(int i) const {
        int ml=0, mu=nbin;
        while( mu - ml > 1 ) {
            int mav = (ml+mu)/2;
            if( i < pix[mav] ) mu = mav; else ml = mav;
        }
        return mu-1;
    };
...
struct EGS_VHP_LOCAL VHP_SliceInfo {
    ...
    ~VHP_SliceInfo() { if( rinfo ) delete [] rinfo; };
    ...
    int getSize() const {
        ...
        if( rinfo ) {
            int nrow = lastr-firstr+1;
            for(int j=0; j<nrow; ++j) result += rinfo[j].getSize();
        }
        return result;
    };
    void readData(istream &in);
...
public:
    ...
    int         nSlice() const { return nslice; };
    int         getOrgan(int islice, int i, int j) const {
        return islice >= 0 && islice < nslice ?
            slices[islice].getOrgan(i,j) : 0;
    };
    const VHP_SliceInfo &getVHP_SliceInfo(int j) const { return slices[j]; };
    const VHP_SliceInfo &operator[](int j) const { return slices[j]; };
    int getSize() const {
        int result = sizeof(nslice) + sizeof(VHP_SliceInfo*);
        if( nslice > 0 ) {
            for(int j=0; j<nslice; ++j) result += slices[j].getSize();
        }
        return result;
    };
    void getPhantomDimensions(int &Nx, int &Ny, int &Nz) {
        Nx = nx; Ny = ny; Nz = nslice;
    };
    ...
    int getOrgan(int ireg) const {
        int k = ireg/nxy;
        if( k < 0 || k >= nslice ) return 0;
        int jj = ireg - k*nx*ny;
        int j = jj/nx; int i = jj - j*nx;
...
public:
      ...
      ~EGS_VoxelGeometry() { if(v) { delete [] v; v = 0; nv = 0; } };
      ...
      void setIndeces(const EGS_Vector &x) {
          ix = (int) (x.x*dxi); if( ix >= nx ) ix = nx-1;
          iy = (int) (x.y*dyi); if( iy >= ny ) iy = ny-1;
          iz = (int) (x.z*dzi); if( iz >= nz ) iz = nz-1;
      };

      int isWhere(const EGS_Vector &x) {
          if( !isInside(x) ) return -1;
          setIndeces(x);
      ...
      int computeIntersections(int ireg, int n, const EGS_Vector &X,
          ...
          if( nv < n ) {
              if( nv > 0 ) delete [] v;
              v = new EGS_VoxelInfo [n]; nv = n;
          }
          int ifirst = 0; EGS_Float t, ttot = 0; EGS_Vector x(X); int imed;
          if( ireg < 0 ) {
              t = 1e30; ireg = howfar(ireg,x,u,t);
              if( ireg < 0 ) return 0;
              isections[0].t = t; isections[0].rhof = 1;
              isections[0].ireg = -1; isections[0].imed = -1;
              ttot = t; ++ifirst; x += u*t;
          }
          int iz = ireg/nxy; int ir = ireg - iz*nxy;
          int iy = ir/nx; int ix = ir - iy*nx;
          EGS_Float uxi, uyi, uzi, nextx, nexty, nextz, sx, sy, sz;
          int dirx, icx, diry, icy, dirz, icz;
          if( u.x > 0 )      {
              uxi = 1/u.x; dirx =  1; nextx = (dx*(ix+1)-x.x)*uxi; sx = uxi*dx;
          }
          else if( u.x < 0 ) {
              uxi = 1/u.x; dirx = -1; nextx = (dx*ix-x.x)*uxi; sx = -uxi*dx;
          }
          else { dirx =  1; nextx = 1e33; sx = 1e33; }
          if( u.y > 0 )      {
              uyi = 1/u.y; diry =  1; nexty = (dy*(iy+1)-x.y)*uyi; sy = uyi*dy;
          }
          else if( u.y < 0 ) {
              uyi = 1/u.y; diry = -1; nexty = (dy*iy-x.y)*uyi; sy = -uyi*dy;
          }
          else { diry =  1; nexty = 1e33; sy = 1e33; }
          if( u.z > 0 )      {
              uzi = 1/u.z; dirz =  1; nextz = (dz*(iz+1)-x.z)*uzi; sz = uzi*dz;
          }
          else if( u.z < 0 ) {
              uzi = 1/u.z; dirz = -1; nextz = (dz*iz-x.z)*uzi; sz = -uzi*dz;
          }
          else  { dirz =  1; nextz = 1e33; sz = 1e33; }
          for(int j=ifirst; j<n; j++) {
              isections[j].ireg = ireg;
              v[j].ix = ix; v[j].iy = iy; v[j].iz = iz;
              int inew;
              if( nextx < nexty && nextx < nextz ) {
                  t = nextx; ix += dirx;
                  if( ix < 0 || ix >= nx ) inew = -1;
                  else {
                      inew = ireg + dirx; nextx += sx;
                  }
              }
              else if( nexty < nextz ) {
                  t = nexty; iy += diry;
                  if( iy < 0 || iy >= ny ) inew = -1;
                  else {
                      inew = ireg + nx*diry; nexty += sy;
                  }
              }
              else {
                  t = nextz; iz += dirz;
                  if( iz < 0 || iz >= nz ) inew = -1;
                  else {
                      inew = ireg + nxy*dirz; nextz += sz;
                  }
              }
              isections[j].t = t;
              if( inew < 0 ) return j+1;
          }
          return ireg >= 0 ? -1 : n;
      };
      ...
      void setIndeces(int ireg) {
          iz = ireg/nxy; int ir = ireg - iz*nxy;
          iy = ir/nx; ix = ir - iy*nx;
      };
      ...
      int howfarIn(int ireg, const EGS_Vector &x, const EGS_Vector &u,
          ...
          if( u.x > 0 ) {
              ...
              if( d <= t ) {
                  t = d; if( (++ix) < nx ) inew = ireg + 1; else inew = -1;
              ...
              if( d <= t ) {
                  t = d; if( (--ix) >= 0 ) inew = ireg - 1; else inew = -1;
                  if( normal ) *normal = EGS_Vector(1,0,0);
              }
          }
          if( u.y > 0 ) {
              ...
              if( d <= t ) {
                  ix = ixs;
                  t = d; if( (++iy) < ny ) inew = ireg + nx; else inew = -1;
              ...
              if( d <= t ) {
                  ix = ixs;
                  t = d; if( (--iy) >= 0 ) inew = ireg - nx; else inew = -1;
                  if( normal ) *normal = EGS_Vector(0,1,0);
              }
          }
          if( u.z > 0 ) {
              ...
              if( d <= t ) {
                  ix = ixs; iy = iys;
                  t = d; if( (++iz) < nz ) inew = ireg+nxy; else inew = -1;
              ...
              if( d <= t ) {
                  ix = ixs; iy = iys;
                  t = d; if( (--iz) >= 0 ) inew = ireg-nxy; else inew = -1;
                  if( normal ) *normal = EGS_Vector(0,0,1);
              }
          }
          return inew;
      ...
      int howfarOut(const EGS_Vector &x, const EGS_Vector &u,
              EGS_Float &t, EGS_Vector *normal=0) {
          EGS_Float tlong = 2*t, d; int inew = -1;
          if( x.x <= xmin && u.x > 0 ) {
              ix = 0; d = (xmin-x.x)/u.x;
          }
          else if( x.x >= xmax && u.x < 0 ) {
              ix = nx-1; d = (xmax-x.x)/u.x;
          }
          else d = tlong;
          if( d <= t ) {
              ...
              if( yy >= ymin && yy <= ymax && zz >= zmin && zz <= zmax ) {
                  t = d;
                  iy = (int) (yy*dyi); if( iy > ny-1 ) iy = ny-1;
                  iz = (int) (zz*dzi); if( iz > nz-1 ) iz = nz-1;
          ...
          if( d <= t ) {
              ...
              if( xx >= xmin && xx <= xmax && zz >= zmin && zz <= zmax ) {
                  t = d;
                  ix = (int) (xx*dxi); if( ix > nx-1 ) ix = nx-1;
                  iz = (int) (zz*dzi); if( iz > nz-1 ) iz = nz-1;
      ...
      EGS_Float hownearIn(const EGS_Vector &x) {
          ...
          t1 = x.x-dx*ix; t2 = dx - t1; tx = t1 < t2 ? t1 : t2;
          t1 = x.y-dy*iy; t2 = dy - t1; ty = t1 < t2 ? t1 : t2;
          t1 = x.z-dz*iz; t2 = dz - t1; tz = t1 < t2 ? t1 : t2;
          return tx < ty && tx < tz ? tx : ty < tz ? ty : tz;
      };

      EGS_Float hownear(int ireg, const EGS_Vector &x) {
          if( ireg >= 0 ) {
              setIndeces(ireg); return hownearIn(x);
          }
          int nc = 0; EGS_Float s1=0, s2=0;
...
struct VHPBox {
    ...
    EGS_Float ymin, ymax;
    EGS_Float zmin, zmax;
    bool isInside(const EGS_Vector &x) { return
    ...
    bool willEnter(const EGS_Vector &x, const EGS_Vector &u, EGS_Float &t) {
        ...
        if( x.x <= xmin && u.x > 0 ) d = (xmin-x.x)/u.x;
        else if( x.x >= xmax && u.x < 0 ) d = (xmax-x.x)/u.x;
        else d = tlong;
        if( d <= t ) {
            EGS_Float yy = x.y + u.y*d, zz = x.z + u.z*d;
            if( yy >= ymin && yy <= ymax && zz >= zmin && zz <= zmax ) {
        ...
        if( x.y <= ymin && u.y > 0 ) d = (ymin-x.y)/u.y;
        else if( x.y >= ymax && u.y < 0 ) d = (ymax-x.y)/u.y;
        else d = tlong;
        if( d <= t ) {
            EGS_Float xx = x.x + u.x*d, zz = x.z + u.z*d;
            if( xx >= xmin && xx <= xmax && zz >= zmin && zz <= zmax ) {
        ...
        if( x.z <= zmin && u.z > 0 ) d = (zmin-x.z)/u.z;
        else if( x.z >= zmax && u.z < 0 ) d = (zmax-x.z)/u.z;
        else d = tlong;
        if( d <= t ) {
            EGS_Float xx = x.x + u.x*d, yy = x.y + u.y*d;
            if( xx >= xmin && xx <= xmax && yy >= ymin && yy <= ymax ) {
    ...
    EGS_Float howfarToOut(const EGS_Vector &x, const EGS_Vector &u) {
        EGS_Float t = 1e35, t1;
        if     ( u.x > 0 ) { t1 = (xmax-x.x)/u.x; if( t1 < t ) t = t1; }
        else if( u.x < 0 ) { t1 = (xmin-x.x)/u.x; if( t1 < t ) t = t1; }
        if     ( u.y > 0 ) { t1 = (ymax-x.y)/u.y; if( t1 < t ) t = t1; }
        else if( u.y < 0 ) { t1 = (ymin-x.y)/u.y; if( t1 < t ) t = t1; }
        if     ( u.z > 0 ) { t1 = (zmax-x.z)/u.z; if( t1 < t ) t = t1; }
        else if( u.z < 0 ) { t1 = (zmin-x.z)/u.z; if( t1 < t ) t = t1; }
        return t;
    };
};
#endif
...
public:
    ...
    int howfar(int imic, int ireg, int &lax, int &lay, int &laz, int &newmic,
        ...
        newmic = imic;
        if( inew < 0 ) {
            int iz = imic/(mx*my); int itmp = imic - iz*mx*my;
            int iy = itmp/mx; int ix = itmp - mx*iy;
            if     ( vg->ix < 0 )       { --lax; if( (--ix) < 0   ) ix = mx-1; }
            else if( vg->ix >= vg->nx ) { ++lax; if( (++ix) >= mx ) ix = 0;    }
            else if( vg->iy < 0 )       { --lay; if( (--iy) < 0   ) iy = my-1; }
            else if( vg->iy >= vg->ny ) { ++lay; if( (++iy) >= my ) iy = 0;    }
            else if( vg->iz < 0 )       { --laz; if( (--iz) < 0   ) iz = mz-1; }
            else if( vg->iz >= vg->nz ) { ++laz; if( (++iz) >= mz ) iz = 0;    }
            newmic = ix + iy*mx + iz*mx*my;
        }
        if( newmed ) *newmed = micros[newmic][inew] ? med_bm : med_tb;
        return inew;
    };
    ...
    int medium(int ireg) const {
        int imic = ireg/nreg; ireg -= imic*nreg;
    ...
    void getIndeces(int ireg, int &imic, int &ix, int &iy, int &iz) {
        imic = ireg/nreg; ireg -= imic*nreg;
        iz = ireg/nxy; iy = (ireg - iz*nxy)/nx; ix = ireg - iz*nxy - iy*nx;
    };
    ...
    int isWhere(int jx, int jy, int jz, const EGS_Vector &x1, int *newmed=0) {
        ...
        if( newmed ) *newmed = medium(imic,iloc);
        return imic*nreg + iloc;
    };

    bool isValid() const { return (vg && micros); };

    void getStepFractions(int imic, int ireg,
            const EGS_Vector &xi, const EGS_Vector &xf,
            EGS_Float &bsc_step, EGS_Float &bm_step ) {
        bsc_step = 0; bm_step = 0; int mict = micros[imic][ireg];
        if( mict ) {
            // a BM voxel => check step fractions
            if( mict == 1 ) bsc_step = 1;      // all BSC
            else if( mict == 2 ) bm_step = 1;  // all BM
            else {
                mict -= 2;
                // get positions in bm_boxes co-ordinate system
                int jz = ireg/nxy; int jy = (ireg-jz*nxy)/nx;
                ...
                // is in the BM box
                if( i_inside && f_inside ) bm_step = 1;
                else {
                    // we have a fraction in BM and remainder in BSC
                    EGS_Vector u(Xf-Xi);
                    EGS_Float t = u.length(); EGS_Float ti = 1/t; u *= ti;
                    if( !i_inside ) {
                    ...
                    if( f_inside ) {
                        ...
                        bsc_step *= ti; bm_step = 1 - bsc_step; return;
                    }
    ...
    void getVolumes(int imic, EGS_Float &tb, EGS_Float &bm, EGS_Float &bsc) {
        tb = v_tb[imic]; bm = v_bm[imic]; bsc = v_bsc[imic];
    };
...
public:
    ...
    bool isInside(const EGS_Vector &x) { return vg->isInside(x); }
    int isWhere(const EGS_Vector &x) {
        int ireg = vg->isWhere(x); if( ireg < 0 ) return ireg;
    ...
    int howfar(int ireg, const EGS_Vector &x, const EGS_Vector &u,
               EGS_Float &t, int *newmed=0, EGS_Vector *normal=0) {
        if( ireg >= 0 ) {
            int voxel = ireg/nr; int ilocal = ireg - voxel*nr;
            vg->setIndeces(voxel);
            EGS_Vector x1(x-EGS_Vector(vg->dx*vg->ix,vg->dy*vg->iy,vg->dz*vg->iz));
            int inew = micro->vg->howfar(ilocal,x1,u,t,normal);
            if( inew == ilocal ) return ireg;
            if( inew < 0 ) {
                if( micro->vg->ix < 0 ) {
                    if( (--vg->ix) < 0 ) return -1;
            ...
            if( newmed ) *newmed = micro->medium(voxel,inew);
            return voxel*nr + inew;
        }
        int voxel = vg->howfar(ireg,x,u,t,normal);
        if( voxel < 0 ) return voxel;
        EGS_Vector x1(x + u*t);
        x1 -= EGS_Vector(vg->dx*vg->ix,vg->dy*vg->iy,vg->dz*vg->iz);
        int ilocal = micro->vg->isWhereFast(x1);
        if( ilocal < 0 ) egsFatal("x=(%g,%g,%g) x+u*t=(%g,%g,%g) x1=(%g,%g,%g)\n",x.x,x.y,x.z,(x+u*t).x,(x+u*t).y,(x+u*t).z,x1.x,x1.y,x1.z);
        if( newmed ) *newmed = micro->medium(voxel,ilocal);
        return voxel*nr + ilocal;
    };

    EGS_Float howfarToOutside(int ireg, const EGS_Vector &x,
            const EGS_Vector &u) { return vg->howfarToOutside(ireg,x,u); };

    EGS_Float hownear(int ireg, const EGS_Vector &x) {
        if( ireg < 0 ) return vg->hownear(ireg,x);
        int voxel = ireg/nr; int ilocal = ireg - voxel*nr;
        vg->setIndeces(voxel);
        EGS_Vector x1(x-EGS_Vector(vg->dx*vg->ix,vg->dy*vg->iy,vg->dz*vg->iz));
...
public:
      ...
      int isWhere(const EGS_Vector &x) {
          ...
          if( !nmicro ) return ireg;
          int iorg = organs->getOrgan(vg->iz,vg->iy,vg->ix);
          int mict = organ_micro[iorg];
          if( mict < 0 ) return ireg; // i.e., no micros in this macro voxel
          EGS_Vector x1(x - EGS_Vector(vg->dx*vg->ix,vg->dy*vg->iy,vg->dz*vg->iz));
      ...
      int medium(int ireg) const {
          ...
          ireg -= nmacro; int mict = ireg/nmax;
      ...
      int computeIntersections(int ireg, int n, const EGS_Vector &X,
          ...
          if( !result ) return result; // no intersections
      ...
      int howfar(int ireg, const EGS_Vector &x, const EGS_Vector &u,
          ...
          if( ireg >= 0 ) {
              // in a micro matrix.
              int ireg1 = ireg - nmacro; int mict = ireg1/nmax;
              ...
              if( ix > 0 && ix < micros[mict]->vg->nx-1 ) lax = (int) aux;
              else if( ix == 0 ) lax = (int) (aux+0.5);
              else               lax = (int) (aux-0.5);
              aux = x.y*vg->dyi;
              if( iy > 0 && iy < micros[mict]->vg->ny-1 ) lay = (int) aux;
              else if( iy == 0 ) lay = (int) (aux+0.5);
              else               lay = (int) (aux-0.5);
              aux = x.z*vg->dzi;
              if( iz > 0 && iz < micros[mict]->vg->nz-1 ) laz = (int) aux;
              else if( iz == 0 ) laz = (int) (aux+0.5);
              else               laz = (int) (aux-0.5);
              ...
              if( inew < 0 ) {
                  ...
                  if( mict < 0 ) { // no.
                      if( newmed ) *newmed = organ_media[iorg];
              ...
              if( newmed ) *newmed = micros[mict]->medium(imic,inew);
              return nmax*mict + inew + nmacro;
          }
          // outside
          int imac = vg->howfarOut(x,u,t,normal);
          if( imac < 0 ) return imac;
          int iorg = organs->getOrgan(vg->iz,vg->iy,vg->ix);
          int mict = organ_micro[iorg];
          if( mict < 0 ) {
              if( newmed ) *newmed = organ_media[iorg];
          ...
          if( newmed ) *newmed = micros[mict]->medium(imic,ilocal);
          return nmax*mict + imic*micros[mict]->nreg + ilocal + nmacro;
      };

      EGS_Float hownear(int ireg, const EGS_Vector &x) {
          if( ireg < nmacro ) return vg->hownear(ireg,x);
          ireg -= nmacro; int mict = ireg/nmax;
          int iloc = ireg - mict*nmax;
          int imic, ix, iy, iz;
          EGS_MicroMatrixCluster *micro = micros[mict];
          ...
          int lax, lay, laz;
          EGS_Float aux = x.x*vg->dxi;
          if( ix > 0 && ix < micro->vg->nx-1 ) lax = (int) aux;
          else if( ix == 0 ) lax = (int) (aux+0.5);
          else               lax = (int) (aux-0.5);
          aux = x.y*vg->dyi;
          if( iy > 0 && iy < micro->vg->ny-1 ) lay = (int) aux;
          else if( iy == 0 ) lay = (int) (aux+0.5);
          else               lay = (int) (aux-0.5);
          aux = x.z*vg->dzi;
          if( iz > 0 && iz < micro->vg->nz-1 ) laz = (int) aux;
          else if( iz == 0 ) laz = (int) (aux+0.5);
          else               laz = (int) (aux-0.5);
          EGS_Vector x1(x - EGS_Vector(vg->dx*lax,vg->dy*lay,vg->dz*laz));
      ...
      int getMaxStep() const {
          if( !nmicro ) return vg->nx+vg->ny+vg->nz+1;
          int nmx=0, nmy=0, nmz=0;
          for(int mict=0; mict<nmicro; ++mict) {
              if( micros[mict]->nx > nmx ) nmx = micros[mict]->nx;
              if( micros[mict]->ny > nmy ) nmy = micros[mict]->ny;
              if( micros[mict]->nx > nmz ) nmx = micros[mict]->nz;
          }
          return vg->nx*nmx+vg->ny*nmy+vg->nz*nmz+1;
      };

      bool isOK() const { return (vg && organs); };

      int getNx() const { return vg->nx; };
      int getNy() const { return vg->ny; };
      int getNz() const { return vg->nz; };
