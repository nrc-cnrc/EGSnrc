...
VHP_OrganData::VHP_OrganData(const char *fname, int slice_min, int slice_max)
    ...
    char endian; data.read(&endian,sizeof(endian));
    ...
    for(int islice=0; islice<nslice; ++islice) {
        if( islice >= slice_min ) {
            ...
            int ni, nj; slices[is++].getSliceDimensions(ni,nj);
            if( ni > nx ) nx = ni;
            if( nj > ny ) ny = nj;
        }
        else {
            VHP_SliceInfo tmp; tmp.readData(data);
        }
        if( islice == slice_max - 1 ) break;
    }
    nxy = nx*ny; nslice = slice_max - slice_min;
    if( data.fail() )
        egsWarning("VHP_OrganData: I/O error while reading data\n");
    else ok = true;
}

VHP_OrganData::~VHP_OrganData() { if( nslice ) delete [] slices; }

void VHP_RowInfo::readData(istream &in) {
    unsigned short n;
    in.read((char *)&n,sizeof(n));
    if( n != nbin ) {
        if( nbin > 0 ) { delete [] pix; delete [] org; nbin = 0; }
    }
    if( n < 1 ) return;
    if( !nbin ) {
        nbin = n; pix = new unsigned short [nbin+1];
...
EGS_VHPGeometry::EGS_VHPGeometry(const char *phantom_file,
    ...
    int j;
    for(j=0; j<256; ++j) {
        organ_media[j] = -1; organ_names[j] = "Undefined";
        organ_micro[j] = -1;
    }
    char buf[1024]; med_data.getline(buf,1023);
    for(j=0; j<norg; ++j) {
        int iorg, imed; char c; med_data >> iorg >> imed >> c;
        med_data.getline(buf,1023);
        if( iorg < 0 || iorg > 255 )
            egsWarning("EGS_VHPGeometry: bad organ number %d on line %d\n",
                    iorg,j+2);
        else organ_media[iorg] = imed;
        organ_names[iorg] = c; organ_names[iorg] += buf;
    }
    int nmed; med_data >> nmed; med_data.getline(buf,1023);
    vector<string> media_names;
    vector<int> media_indeces;
    for(j=0; j<nmed; ++j) {
        med_data.getline(buf,1023); string med(buf);
    ...
    for(j=0; j<256; ++j) {
        if( organ_media[j] >= 0 ) {
            ...
            else organ_media[j] = media_indeces[organ_media[j]];
        }
    }
    EGS_Float dx, dy, dz; int nx, ny, nz;
    organs->getVoxelSizes(dx,dy,dz); organs->getPhantomDimensions(nx,ny,nz);
    vg = new EGS_VoxelGeometry(nx,ny,nz,dx,dy,dz);
    nreg = nx*ny*nz; nmacro = nreg;
    ...
    while( !input->getInput("micro matrix",minput) ) {
        ...
        if( !mcluster->isValid() ) {
            egsWarning("failed to construct micro cluster from %s\n",
                    fname.c_str()); delete mcluster;
        }
        else {
            bool ok = true; vector<int> orgs;
            for(int i=1; i<minput.size(); ++i) {
                S_STREAM s(minput[i].c_str()); int org; s >> org;
                if( s.fail() ) {
                    egsWarning("EGS_VHPGeometry::setMicros: invalid input %s "
                          "for micro matrix %s\n",minput[i].c_str(),
                          minput[0].c_str()); ok = false;
                }
                else {
                    if( org < 0 || org > 255 ) {
            ...
            else delete mcluster;
        }
    }
    if( mv.size() > 0 ) {
        if( nmicro > 0 ) {
            for(int i=0; i<nmicro; ++i) delete micros[i];
            delete [] micros;
        }
        nmicro = mv.size();
        ...
        for(int i=0; i<nmicro; ++i) {
            ...
            if( nm*nr > nmax ) nmax = nm*nr;
        }
        nmacro = vg->nxy*vg->nz;
        EGS_I64 ntot = nmax; ntot *= nmicro; ntot += nmacro;
...
EGS_VHPGeometry::~EGS_VHPGeometry() {
    if( organs ) delete organs;
    if( vg ) delete vg;
    if( nmicro > 0 ) {
        for(int j=0; j<nmicro; ++j) delete micros[j];
        delete [] micros;
    }
...
EGS_MicroMatrixCluster::EGS_MicroMatrixCluster(
    ...
    mx = Mx; my = My; mz = Mz;
    mxy = mx*my; nmic = mxy*mz;
    micros = new unsigned char* [nmic];
    in.read((char *)&Mx,sizeof(Mx));
    in.read((char *)&My,sizeof(My));
    in.read((char *)&Mz,sizeof(Mz));
    nx = Mx; ny = My; nz = Mz; nxy = nx*ny; int nxyz = nxy*nz, j; nreg = nxyz;
    {for(int j=0; j<nmic; ++j) {
        ...
        if( in.fail() ) {
            ...
            for(int i=0; i<=j; ++i) delete [] micros[j];
            delete [] micros; micros = 0; return;
        }
    }}
    ...
    for(j=0; j<nmic; ++j) {
        int iz = j/mxy; int iy = (j-iz*mxy)/mx; int ix = j-iz*mxy-iy*mx;
        int ixm = ix-1; if( ixm < 0 ) ixm = mx-1;
        int ixp = ix+1; if( ixp >= mx ) ixp = 0;
        int iym = iy-1; if( iym < 0 ) iym = my-1;
        int iyp = iy+1; if( iyp >= my ) iyp = 0;
        int izm = iz-1; if( izm < 0 ) izm = mz-1;
        int izp = iz+1; if( izp >= mz ) izp = 0;
        ...
        tot_tb += tb_vol; tot_bm += bm_vol; tot_bsc += bsc_vol;
    }
...
EGS_VHP_EXPORT EGS_BaseGeometry* createGeometry(EGS_Input *input) {
    ...
    if( !err && type == "micro_test" ) {
        ...
        if( err || vs.size() != 3 ) {
            egsWarning(vhp_error_msg1,"'voxel sizes'"); ok = false;
        }
        EGS_Float bsc_t;
        err = input->getInput("BSC thickness",bsc_t);
        if( err ) {
            egsWarning(vhp_error_msg1,"'BSC thickness'"); ok = false;
        }
        string tb_medium, bm_medium;
        int err1 = input->getInput("TB medium",tb_medium);
        int err2 = input->getInput("BM medium",bm_medium);
        if( err1 ) egsWarning(vhp_error_msg1,"'TB medium'");
        if( err2 ) egsWarning(vhp_error_msg1,"'BM medium'");
        if( err1 || err2 ) ok = false;
        string micro_data;
        err = input->getInput("micro data file",micro_data);
        if( err ) { egsWarning(vhp_error_msg1,"'micro data file'"); ok = false;}
        if( !ok ) return 0;
    ...
    else result = new EGS_VHPGeometry(phantom.c_str(),media.c_str());
    if( !result->isOK() ) {
