...
EGS_cSpheres::EGS_cSpheres(int ns, const EGS_Float *radius,
    ...
    if(ns>0) {
        ...
        R2=new EGS_Float [ns]; R=new EGS_Float [ns];

        // ... and sphere radii
        for(int i=0;i<ns;i++) {
            R2[i]=radius[i]*radius[i]; R[i]=radius[i];
        }
...
bool EGS_cSpheres::isInside(const EGS_Vector &x) {
    EGS_Vector tmp(x-xo); EGS_Float r_sq=tmp.length2();
    if(r_sq>R2[nreg-1]) return false;
    return true;
}

int EGS_cSpheres::isWhere(const EGS_Vector &x) {
    EGS_Vector tmp(x-xo); EGS_Float r_sq=tmp.length2();
    if(r_sq>R2[nreg-1]) return -1;
    if(r_sq<R2[0]) return 0;
 ...
 */if(r_sq<R2[0]) return 0;
  ...
  while(os-is>1) {
      ms=(is+os)/2;
      if(r_sq<=R2[ms]) os=ms; else is=ms;
  }
  return os;
}
...
EGS_Float EGS_cSpheres::howfarToOutside(int ireg, const EGS_Vector &x,
    ...
    if( R2b2 <= 0 ) return 0; // outside within precision
            ...
            const EGS_Vector &u, EGS_Float &t, int *newmed, EGS_Vector *normal)
{
  ...
  if(ireg>=0) {
      ...
      if( aa >= 0 || !ireg ) {
           ...
           */
          R2b2 = R2[ireg] - bb2;
          if( R2b2 <= 0 && aa > 0 ) d = 1e-15; // hopefully a truncation problem
          else {
              tmp = aa2 + R2b2;
              if( tmp > 0 ) tmp = sqrt(tmp);
              else {
                  if( tmp < -1e-2 ) {
          ...
          if( direction_flag >= nreg ) direction_flag = -1;
      }
      else {
  ...
  last_x = x; last_u = u; last_ireg = ireg; last_d = d; last_t = t;
  last_aa = aa; last_bb2 = bb2; last_R2b2 = R2b2; last_tmp = tmp;
#endif

  // check desired step size against this d
  if(d<=t) {
      t=d;
      if( newmed ) if( direction_flag >= 0 ) *newmed = medium(direction_flag);
                   else *newmed = -1;
      if( normal ) {
          EGS_Vector n(xp + u*d); *normal = n*(1/rad);
      }
...
EGS_Float EGS_cSpheres::hownear(int ireg, const EGS_Vector &x)
{
  ...
  if(ireg>=0) {
      d=R[ireg]-r;
      if(ireg) { EGS_Float dd=r-R[ireg-1]; if(dd<d) d=dd; }
  }
  else d=r-R[nreg-1];
...
EGS_SPHERES_EXPORT EGS_BaseGeometry* createGeometry(EGS_Input *input) {
    ...
    for(int j=0; j<radii.size(); j++) r[j] = radii[j];
