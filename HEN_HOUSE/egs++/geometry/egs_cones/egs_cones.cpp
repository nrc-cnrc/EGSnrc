        ...
        if( all ) {
            for(int j=0; j<nl; j++) {
                for(int i=0; i<nr[j]; i++)
                    if( !cones[j][i]->deref() ) delete cones[j][i];
                delete [] cones[j];
            }
            nl = 0;
        }
        delete [] cones; delete [] pos; delete [] nr; delete [] flag;
        nltot = 0;
    }
}
...
void EGS_ConeStack::resize() {
    ...
    for(int j=0; j<nl; j++) {
        new_nr[j] = nr[j]; new_flag[j] = flag[j];
        new_pos[j] = pos[j]; new_cones[j] = cones[j];
    }
    if( nl > 0 ) new_pos[nl] = pos[nl];
    clear(false);
    nr = new_nr; pos = new_pos; flag = new_flag; cones = new_cones;
    nltot = nnew;
}
...
void EGS_ConeStack::addLayer(EGS_Float thick, const vector<EGS_Float> &rtop,
    ...
    if( nl >= nltot ) resize();
    if( !nl ) {
        pos[nl] = xo*a; Rout = rbottom[this_nr-1];
        Rout2 = Rout*Rout;
        if( fabs(rtop[this_nr-1]-Rout) > 1e-5 ) same_Rout = false;
    }

    cones[nl] = new EGS_SimpleCone * [this_nr];
    nr[nl] = this_nr; pos[nl+1] = pos[nl] + thick;
    ...
    for(int ir=0; ir<this_nr; ir++) {
        ...
        cones[nl][ir]->ref();
        if( ir == this_nr-1 ) {
            if( fabs(rbottom[this_nr-1]-Rout) > 1e-5 ) same_Rout = false;
            if( fabs(Rtop-Rout) > 1e-5 ) same_Rout = false;
        }
    }
    if( same_radii ) {
        flag[nl-1] += 2; flag[nl] = 1;
    } else flag[nl] = 0;
    if( this_nr > nmax ) nmax = this_nr;
    ++nl; nreg = nl*nmax;
...
void EGS_ConeStack::printInfo() const {
    ...
    for(int il=0; il<nl; il++) {
        ...
        int i; EGS_Vector x(xo+a*(pos[il]-pos[0]));
        for(i=0; i<nr[il]; i++)
...
void EGS_SimpleCone::printInfo() const {
    ...
    if( open ) egsInformation(" cone is open\n");
...
void EGS_ConeSet::printInfo() const {
    ...
    for(int j=0; j<nc; j++) egsInformation("%g ",180*atan(gamma[j])/M_PI);
    egsInformation("\n=====================================================\n");
}
...
EGS_CONES_EXPORT EGS_BaseGeometry* createGeometry(EGS_Input *input) {
    ...
    if( type == "EGS_ConeStack" ) {
        ...
        while( (layer = input->takeInputItem("layer")) ) {
            ...
            if( err )
                ...
                if( err || err1 ) egsWarning("  --> layer ignored\n");
    ...
    vector<EGS_Float> tmp; EGS_Vector Xo;
    err = input->getInput("apex",tmp);
    if( err || tmp.size() != 3 )
        egsWarning("createGeometry(cones): no 'apex' input, "
                   "assuming (0,0,0)\n");
    else { Xo.x = tmp[0]; Xo.y = tmp[1]; Xo.z = tmp[2]; }
    tmp.clear();

    // get cone axis
    EGS_Vector axis(0,0,1); err = input->getInput("axis",tmp);
    if( err || tmp.size() != 3 )
        egsWarning("createGeometry(cones): no 'axis' input, assuming "
                   "(0,0,1)\n");
    else { axis.x = tmp[0]; axis.y = tmp[1]; axis.z = tmp[2]; }
    ...
    if( input->compare(type,"EGS_ConeSet") ) {
        ...
        for(int j=0; j<nc; j++) {
            ...
            EGS_Float a = angles[j]; if( !is_radian ) a *= (M_PI/180);
            if( a >= M_PI/2 ) {
        ...
        if( input->compare(type,"EGS_SimpleCone") ) {
            EGS_Float height; EGS_Float *h = 0;
            ...
            EGS_Float *dist=0; int nc=1;
            if( !err && d.size() > 0 ) {
                dist = new EGS_Float [d.size()];
                for(int j=0; j<d.size(); j++) dist[j] = d[j];
                nc = 1 + d.size();
            }
