  ...
  public:
    ...
    */
    ...
    bool isInside(const EGS_Vector &x) {
        ...
        if(rho_sq>R2[nreg-1]) return false;
        return true;
    };
    ...
    int isWhere(const EGS_Vector &x) {
        EGS_Vector rc(x-xo);
        EGS_Float rp=a*rc, rho_sq=rc.length2()-rp*rp;
        if(rho_sq>R2[nreg-1]) return -1;
        if(rho_sq<R2[0]) return 0;
    ...
    int inside(const EGS_Vector &x)
    {
      ...
      if(rho_sq>R2[nreg-1]) return -1;  // outside all cylinders
      if(rho_sq<R2[0]) return 0;  // inside central cylinder

      // find particle region
      int ic=0,oc=nreg,ms;
      while(oc-ic>1)
      { ms=(ic+oc)/2;
        if(rho_sq<R2[ms]) oc=ms;
        else ic=ms;
      }
      return oc;
    };
    ...
    EGS_Float howfarToOutside(int ireg, const EGS_Vector &x,
        ...
        if( ireg < 0 ) return 0;
        EGS_Float up=a*u;
        if(fabs(up)>=1) return 1e30; // parallel to axis
        EGS_Float A=1-up*up;
        EGS_Vector rc(x-xo); EGS_Float rcp=a*rc, urc=u*rc;
        EGS_Float C=rc.length2()-rcp*rcp-R2[nreg-1];
        if( C >= 0 ) return 0;  // outside within precision
    ...
    int howfar(int ireg, const EGS_Vector &x, const EGS_Vector &u,
        ...
        // projections
        double up=a*u;
        if(fabs(up)>=1) return ireg; // parallel to cylinder axis
        ...
        last_x = x; last_u = u; last_B = B; last_A = A;
        last_ireg = ireg; last_dir = dir; last_t = t; last_d = d;
#endif

        // correct t-step
        if(d<t) {
            t=d;
            if( newmed ) if( dir >= 0 ) *newmed = medium(dir); else *newmed=-1;
            if( normal ) {
                EGS_Vector n(rc + u*t - a*(rcp+up*t)); *normal = n*(1/rad);
            }
            return dir;
        }
        return ireg;
    ...
    { EGS_BaseGeometry::printInfo();
      ...
      for(int j=0; j<nreg; j++) egsInformation("%g ",R[j]);
      egsInformation("\n");
      egsInformation("=====================================================\n");
    };
};
