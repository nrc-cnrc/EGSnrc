    ...
    */
    ...
    int howfar(int ireg, const EGS_Vector &x, const EGS_Vector &u,
        ...
        if( open ) {
            ...
            if( !hit ) return ireg;
            if( newmed ) *newmed = !ireg ? -1 : med;
            if( normal ) *normal = p->getNormal(v);
            return !ireg ? -1 : 0;
        }
        EGS_Float up = a*u, d = p->distance(x);
        if( !ireg ) { // inside
            EGS_Float tt = 1e30; int inew = ireg;
            if( up > 0 ) tt = (d2 - d)/up; else tt = (d1 - d)/up;
            if( tt <= t ) {
                t = tt; inew = -1;
                if( normal ) *normal = up>0 ? a*(-1) : a;
                if( newmed ) *newmed = -1;
            }
        ...
        if( d < d1 || d > d2 ) {
            EGS_Float tt = 1e30;
            if( d < d1 && up > 0 ) tt = (d1 - d)/up;
            else if( d > d2 && up < 0 ) tt = (d2 - d)/up;
            if( tt < t ) {
                EGS_Vector xp(x + u*tt);
                if( p->isInside2D(xp) ) {
                    t = tt; if( newmed ) *newmed = med;
                    if( normal ) *normal = up>0 ? a*(-1) : a;
                    return 0;
                }
            }
    ...
    EGS_Float hownear(int ireg, const EGS_Vector &x) {
        ...
        if( !ireg ) { // inside
            ...
            if( p->isInside2D(x) ) tperp = t;
            else tperp = sqrt(tperp*tperp + t*t);
        }
        return tperp;
    };
