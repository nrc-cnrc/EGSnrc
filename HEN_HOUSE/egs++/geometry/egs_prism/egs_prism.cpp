...
EGS_PRISM_EXPORT EGS_BaseGeometry* createGeometry(EGS_Input *input) {
    ...
    if( !err && tmp.size() == 2 ) open = false;
    bool p_open = false; int itmp;
    err = input->getInput("open triangle",itmp);
    if( !err && itmp == 1 ) p_open = true;
    if( input->compare(type,__prismX) || input->compare(type,__prismY)
        ...
        for(int j=0; j<np; j++) points.push_back(EGS_2DVector(p[2*j],p[2*j+1]));
        if( open ) {
        ...
        if( aux.length2() > 1e-6 ) points.push_back(points[0]);
        np = points.size();
        ...
        if( open ) g = new EGS_Prism(new EGS_Polygon(p2,pro,p_open));
        else g = new EGS_Prism(new EGS_Polygon(p2,pro,p_open),tmp[0],tmp[1]);
    }
    g->setName(input);
    g->setMedia(input);
    g->setLabels(input);
