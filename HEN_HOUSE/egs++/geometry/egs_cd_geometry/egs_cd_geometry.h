...
public:
      ...
      EGS_CDGeometry(EGS_BaseGeometry *G1, EGS_BaseGeometry **G,
          ...
          nmax = 0; new_indexing = false;
          reg_to_base = 0; local_start = 0;
          nbase = G1->regions();
          ...
          if( !nmax ) nmax = 1;
          nreg = nbase*nmax; is_convex = false;
          if( indexing ) setUpIndexing();
          setHasRhoScaling();
      };
      ...
      int medium(int ireg) const {
          ...
          */
          int ibase, ilocal;
          if( new_indexing ) {
              ibase = reg_to_base[ireg]; ilocal = ireg - local_start[ibase];
          }
          else {
              ibase = ireg/nmax; ilocal = ireg-ibase*nmax;
          }
      ...
      bool isRealRegion(int ireg) const {
          ...
          int ibase, icd;
          if( new_indexing ) {
              ibase = reg_to_base[ireg]; icd = ireg-local_start[ibase];
          }
          else { ibase = ireg/nmax; icd = ireg - ibase*nmax; }
      ...
      bool isInside(const EGS_Vector &x) {
          ...
          if( g[ibase] ) return g[ibase]->isInside(x);
          return true;
      };
      ...
      int howfar(int ireg, const EGS_Vector &x, const EGS_Vector &u,
          ...
          if( ireg >= 0 ) {
              ...
              if( ibase_new != ibase ) {
                  ...
                  if( ibase_new < 0 ) return ibase_new;
                  ...
                  if( g[ibase_new] ) {
                      ...
                      if( newmed ) *newmed = g[ibase_new]->medium(icd_new);
          ...
          if( ibase >= 0 ) {
              ...
              if( icd >= 0 ) {
                  ...
                  tb = 1e30; int ixnew = howfar(ixold,x,u,tb,0,pn);
                  ...
                  // distance.
                  EGS_Float t1=1e30, t2 = 1e30; int ibase_n, ic_n=0;
                  ...
                  if( ibase_n < 0 && t1 < 1e-4 ) {
                      ...
                      EGS_Vector xtmp(x + u*t1); tb = t-t1;
                      ibase = bg->howfar(ibase_n,xtmp,u,tb,pmednew,pn);
                      if( ibase_n < 0 ) return ibase_n; // no, so just return.
                      // yes, so transport to entry point and follow normal outside logic
                      tb += t1; ttot = tb; ibase = ibase_n; first_time = false;
                      goto do_checks;
                  }
                  ...
                  if( g[ibase] ) {
                      ...
                      if( ic_n < 0 && t2 < 1e-4 ) {
                          ...
                          if( ibase < 0 ) {
                              ...
                              if( ibase < 0 ) return ibase;
                              tb += t2; ttot = tb; first_time = false;
                          } else { tb = t2; ttot = tb; }
                          goto do_checks;
                      }
                  }
                  if( t1 < 1e-4 && ibase_n >= 0 && g[ibase_n] ) {
                      ...
                      if( icdx < 0 ) {
                          tb = t1; ttot = tb; ibase = ibase_n; first_time = true;
                          goto do_checks;
                      }
                  }
                  if( t1 > 1e-3 && t2 > 1e-3 ) {
              ...
              if( icd >= 0 ) return ireg;
              tb = 0; ttot = 0;
          } // yes, we are.
          else {
              // no, we are not. Check if we will enter the base geometry.
              first_time = false;
              tb = t; ibase = bg->howfar(ireg,x,u,tb,pmednew,pn);
              if( ibase < 0 ) return ibase; // no, we will not.
...
do_checks:
          ...
          while(1) {
              ...
              if( !first_time ) {// already howfar-checked base geometry
                  ...
                  if( icd >= 0 ) {
                      ...
                      // region.
                      t = ttot;
                      if( newmed ) *newmed = g[ibase]->medium(icd);
                      if( normal ) *normal = n;
      ...
      int getMaxStep() const {
          ...
          for(int j=0; j<bg->regions(); ++j) {
              if( g[j] ) nstep += g[j]->getMaxStep(); else ++nstep;
          }
          return nstep + 1;
      }
      ...
      EGS_Float getRelativeRho(int ireg) const {
          if( ireg < 0 || ireg >= nbase*nmax ) return 1;
          int ibase = ireg/nmax;
...
private:
      ...
      void setHasRhoScaling() {
          ...
          if( bg->hasRhoScaling() ) { has_rho_scaling = true; return; }
          for(int j=0; j<nbase; j++) {
              if( g[j] ) {
                  if(g[j]->hasRhoScaling()) {
                      has_rho_scaling = true; return; }
              }
          }
      };
