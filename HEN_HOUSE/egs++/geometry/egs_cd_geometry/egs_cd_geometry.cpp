...
void EGS_CDGeometry::setUpIndexing() {
    ...
    for(j=0; j<nbase; j++) {
        ...
        if( g[j] ) {
            int n = g[j]->regions();
            for(int i=0; i<n; i++) reg_to_base[ir++] = j;
        }
        else reg_to_base[ir++] = j;
    }
    new_indexing = true; nreg = nr;
}
...
EGS_CDGEOMETRY_EXPORT EGS_BaseGeometry* createGeometry(EGS_Input *input) {
    ...
    EGS_BaseGeometry **G = new EGS_BaseGeometry* [nreg]; int j;
    for(j=0; j<nreg; j++) G[j] = 0;
    int ng = 0; string aux; err = 0;
    while( (ij = input->takeInputItem("set geometry")) != 0 ) {
        vector<string> aux; ij->getInput("set geometry",aux);
        int istart, iend; string name;
        bool is_ok = true;
        if( aux.size() == 2 ) {
            string auxx = aux[0]; auxx += ' '; auxx += aux[1]; auxx += ' ';
    ...
    if( err ) {
        ...
        for(j=0; j<nreg; j++) {
            if( G[j] ) { if( !G[j]->deref() ) delete G[j]; }
        }
        delete [] G[j]; return 0;
    }
    if( !ng )
...
void EGS_CDGeometry::getLabelRegions (const string &str, vector<int> &regs) {
    ...
    for (int i=0; i<nbase; i++) {
        ...
        gregs.clear();
        if (g[i]) g[i]->getLabelRegions(str, gregs);
