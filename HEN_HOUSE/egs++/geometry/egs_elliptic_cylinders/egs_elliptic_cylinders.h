...
public:
    ...
    */
    ~EGS_EllipticCylindersT() {
        if(nreg > 0) {
            delete [] ax; delete [] axi;
            delete [] ay; delete [] ayi;
            delete [] sx; delete [] sy;
            delete [] is_cyl;
        }
    };
    ...
    */
    EGS_EllipticCylindersT(int nc, const EGS_Float *x_rad,
        ...
        if(nc>0) {
            ax = new EGS_Float [nc]; axi = new EGS_Float [nc];
            ay = new EGS_Float [nc]; ayi = new EGS_Float [nc];
            sx = new EGS_Float [nc]; sy = new EGS_Float [nc];
            is_cyl = new bool [nc];
            for(int i=0;i<nc;i++) {
                ax[i]=x_rad[i]; axi[i] = 1/ax[i];
                ay[i]=y_rad[i]; ayi[i] = 1/ay[i];
                EGS_Float z = ay[i]/ax[i];
                if( fabs(z-1) > 1e-4 ) {
                    sx[i] = 1/(z*z-1)/ax[i]; sy[i] = z*sx[i];
                    is_cyl[i] = false;
                } else is_cyl[i] = true;
            }
            nreg=nc;
    ...
    int howfar(int ireg, const EGS_Vector &x, const EGS_Vector &u,
        ...
        EGS_Vector xp(x-xo);
        int inew = ireg; int iold = inew < 0 ? nreg : ireg;
        int ihit = ireg;
        if( ireg < 0 || ireg > 0 ) {
        ...
        if( ireg >= 0 && t > 0 ) {
            ...
            if( u2 > 0 ) {
                ...
                if( d <= t ) {
                    t = d; inew = ireg+1; ihit = ireg;
                }
            }
        }
        if( inew == ireg ) return inew;
        ...
        if( normal ) {
            ...
            *normal = Ax.normal()*(xx*ay[ihit]*ay[ihit]) +
            ...
            normal->normalize();
            if( inew > iold ) *normal *= (-1);
        }
        if( inew >= nreg ) return -1;
    ...
    void printInfo() const {
        ...
        for(j=0; j<nreg; j++) egsInformation(" %g",ax[j]);
        egsInformation("\n");
        egsInformation(" radii along 'y' =");
        for(j=0; j<nreg; j++) egsInformation(" %g",ay[j]);
        egsInformation("\n");
        egsInformation("===================================================\n");
    };
...
private:
    ...
    EGS_Float hownear(int ireg, EGS_Float xx, EGS_Float xy) {
        ...
        double y1 = ay[ireg]*sqrt(1-x1*x1) - xy; x1 = ax[ireg]*x1-xx;
