...
struct EGS_ENVELOPEG_LOCAL EnvelopeAux {
    ...
    int              *regs;
    EnvelopeAux() : nreg(0) {};
    ~EnvelopeAux() { if(nreg>0) delete [] regs; };
};
...
EGS_EnvelopeGeometry::EGS_EnvelopeGeometry(EGS_BaseGeometry *G,
    ...
    if( !G ) egsFatal("EGS_EnvelopeGeometry: base geometry must not be null\n");
    g = G; g->ref(); new_indexing = false;
    nbase = g->regions();
    n_in = geoms.size(); nmax = 1;
    int nreg_inscribed = 0;
    if( n_in > 0 ) {
        geometries = new EGS_BaseGeometry * [n_in];
        for(int j=0; j<n_in; j++) {
            geometries[j] = geoms[j]; geometries[j]->ref();
    ...
    if( newindexing ) {
        ...
        for(int j=0; j<n_in; j++) {
            ...
            for(int i=0; i<n; i++) reg_to_inscr[ir++] = j;
        }
        nreg = nbase + ir;
    }
}
...
EGS_FastEnvelope::EGS_FastEnvelope(EGS_BaseGeometry *G,
         ...
         int newindexing) :
         EGS_BaseGeometry(Name), reg_to_inscr(0), local_start(0) {
    if( !G ) egsFatal("EGS_FastEnvelope: base geometry must not be null\n");
    g = G; g->ref();
    n_in = fgeoms.size(); nmax = 1; new_indexing = false;
    if( !n_in ) egsFatal("EGS_FastEnvelope: no inscribed geometries!\n");
    geometries = new EGS_BaseGeometry * [n_in];
    nbase = g->regions();
    int *iaux = new int [nbase]; int j;
    for(j=0; j<nbase; j++) iaux[j] = 0;
    int nlist = 0;
    int nreg_inscribed = 0;
    for(j=0; j<fgeoms.size(); j++) {
        ...
        for(int i=0; i<fgeoms[j]->nreg; i++) {
            ...
            if( k >= 0 && k < nbase ) { ++iaux[k]; ++nlist; }
        }
    }
    n_start = new int [nbase+1];
    glist = new int [nlist]; int ilist=0;
    for(j=0; j<nbase; j++) {
        n_start[j] = ilist;
        if( iaux[j] > 0 ) {
            for(int l=0; l<fgeoms.size(); l++) {
                for(int i=0; i<fgeoms[l]->nreg; i++) {
                    ...
                    if( k == j ) glist[ilist++] = l;
                }
            }
        }
    }
    n_start[nbase] = ilist;
    nreg = nbase + n_in*nmax; is_convex = g->isConvex();
    has_rho_scaling = g->hasRhoScaling();
    if( !has_rho_scaling ) {
        for(int j=0; j<n_in; j++) {
            if( geometries[j]->hasRhoScaling() ) {
...
void EGS_EnvelopeGeometry::getLabelRegions (const string &str, vector<int> &regs) {
    ...
    for (int i=0; i<n_in; i++) {
        ...
        if (new_indexing) shift = local_start[i];
        else shift = nbase+i*nmax;
