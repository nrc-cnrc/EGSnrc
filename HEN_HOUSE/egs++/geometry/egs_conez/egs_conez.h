...
class EGS_CONEZ_EXPORT EGS_ConezT : public EGS_BaseGeometry
{
  ...
  public:
    ...
    ~EGS_ConezT()
    { if(nreg) { delete [] theta; delete [] cos_t;
                                  delete [] cos2_t;
                                  delete [] sin_t;
                                  delete [] sin2_t; } };
    ...
    {
      if(nc>0)
      {  theta=new EGS_Float [nc]; cos_t=new EGS_Float [nc];
...
*/
if(region_test==1 && cos2_phi<cos2_t[nreg-1]) region_test=-1;
if(region_test==1 && cos2_phi>cos2_t[0]) region_test=0;
...
*/
EGS_Float tmp;
if(rcp<0) tmp=acos(sqrt(cos2_phi))*180/M_PI+90;
else tmp=acos(sqrt(cos2_phi))*180/M_PI;
...
return region_test;
    ...
    bool isInside(const EGS_Vector &x) { return false; }
    int  isWhere(const EGS_Vector &x) { return -1; }
...
//egsInformation("--> %d\n",new_region);
          ...
          { EGS_Float A=cos2_t[ireg_inner]-up*up;
            ...
            EGS_Float C=rc.length2()*cos2_t[ireg_inner]-rcp*rcp;
                   ...
                   // OR 2 unphysical intersections
              {
                ...
                if(rcp>0 && (up<0 && -d*up>rcp)) return -1;
                if(rcp<0 && (up<0 || -d*up>rcp)) return -1;
              }
      ...
      if(new_region>=nreg) new_region=-1;
      // correct t-step
      egsInformation("about to return: new_region = %d d = %g\n",new_region,d);
      if(d<t) {
          t=d; return new_region;
      }
      return ireg;
    };
...
*/
    ...
    { EGS_BaseGeometry::printInfo();
      ...
      for(int j=0; j<nreg; j++) egsInformation("%g ",theta[j]*180/M_PI);
      egsInformation("\n===================================================\n");
    };
};
...
class EGS_CONEZ_LOCAL Projector {
  public:
    ...
    EGS_Vector operator*(const EGS_Float f)
      const {return EGS_Vector(a.x*f,a.y*f,a.z*f); };
