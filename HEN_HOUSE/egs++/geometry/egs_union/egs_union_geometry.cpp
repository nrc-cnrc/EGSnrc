...
EGS_UnionGeometry::EGS_UnionGeometry(const vector<EGS_BaseGeometry *> &geoms,
    ...
    for(j=0; j<ng; j++) {
        ...
        if( !has_rho_scaling ) has_rho_scaling = g[i]->hasRhoScaling();
    }
    delete [] order;
...
EGS_UNIONG_EXPORT EGS_BaseGeometry* createGeometry(EGS_Input *input) {
    ...
    vector<EGS_BaseGeometry *> geoms; vector<string> gnames;
    ...
    if( !err ) {
        if( pri.size() == geoms.size() ) {
            p = new int [pri.size()];
            for(int i=0; i<pri.size(); i++) p[i] = pri[i];
        }
