...
public:
      ...
      */
      ...
      bool isRealRegion(int ireg) const {
          ...
          int j = ireg/nmax; return g[j]->isRealRegion(ireg-j*nmax);
      };
              ...
              if( inew < 0 ) {
                  ...
                  for(int j=jg+1; j<ng; j++) {
                      ...
                      if( ii >= 0 ) {
                          ...
                          jgnew = j; inew = ii; break;
                      }
                  }
              }
              if( inew < 0 ) return inew;
              if( newmed ) *newmed = g[jgnew]->medium(inew);
      ...
      int getMaxStep() const {
          int nstep = 1;
          for(int j=0; j<ng; ++j) nstep += g[j]->getMaxStep();
          return nstep;
      };
