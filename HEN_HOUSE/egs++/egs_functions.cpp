...
bool EGS_EXPORT egsStoreI64(ostream &data, EGS_I64 n) {
    EGS_I64 i1 = n; i1 /= 1000000000; EGS_I64 i2 = n % 1000000000;
    data << " " << (int) i1 << " " << (int) i2;
...
bool EGS_EXPORT egsGetI64(istream &data, EGS_I64 &n) {
    int i1, i2; data >> i1 >> i2;
    if( data.eof() || !data.good() ) return false;
    n = i1; n *= 1000000000; n += i2;
    return true;
}
...
static void EGS_LOCAL __egs_default_information(const char *msg, ...) {
    ...
    if( a ) {
        vsprintf(__egsf_write_buf,msg,ap); a->appInformation(__egsf_write_buf);
    }
    else {
        vfprintf( egs_info_fp, msg, ap ); fflush(egs_info_fp);
    }
    va_end(ap);
}
...
static void EGS_LOCAL __egs_default_warning(const char *msg, ...) {
    ...
    EGS_Application *a = EGS_Application::activeApplication();
    if( a ) {
        vsprintf(__egsf_write_buf,msg,ap); a->appWarning(__egsf_write_buf);
    }
    else {

        vfprintf( egs_warning_fp, msg, ap ); fflush(egs_warning_fp);
    }
    va_end(ap);
}
...
static void EGS_LOCAL __egs_default_error(const char *msg, ...) {
    ...
    EGS_Application *a = EGS_Application::activeApplication();
    if( a ) {
        vsprintf(__egsf_write_buf,msg,ap); va_end(ap);
        a->appFatal(__egsf_write_buf);
    }
    else {
        vfprintf( egs_error_fp, msg, ap ); fflush(stderr);
        va_end(ap); exit(1);
    }
}
...
EGS_InfoFunction egsSetInfoFunction(EGS_InfoType t, EGS_InfoFunction func) {
    ...
    switch (t) {
        case Information: res = egsInformation; egsInformation = func; break;
        case Warning: res = egsWarning; egsWarning = func; break;
        case Fatal: res = egsFatal; egsFatal = func; break;
        default: egsWarning("Unknown info function type\n"); res = 0;
    }
    return res;
}
...
void egsSwapBytes(int *n) { __egs_swap_bytes<int>(n); }
void egsSwapBytes(float *n) { __egs_swap_bytes<float>(n); }

void egsSwapBytes(short *n) {
    char *c = (char *) n;
    char tmp=c[0]; c[0]=c[1]; c[1]=tmp;
}

string egsJoinPath(const string &first, const string &second) {
    int n = first.size()-1; char c = first[n];
    string result(first);
    if( c == '/' && c != __egs_fs ) result[n] = __egs_fs;
    if( result[n] != __egs_fs ) result += __egs_fs;
    result += second;
    return result;
}
...
string egsStripPath(const string &aname) {
    ...
    for(j=aname.size()-1; j>=0; j--) {
        if( aname[j] == '/' || aname[j] == __egs_fs ) { j++; break; }
    }
    if( j < 0 ) return aname;
    string result;
    if( j >= 0 ) {
        while( j < aname.size() ) result += aname[j++];
    }
    return result;
}
...
string egsSimplifyCVSKey(const string &key) {
    if( key.size() < 2 ) return key;
    int js; for(js=0; js<key.size(); js++) if( key[js] == '$' ) break;
    if( js >= key.size() ) return key;
    int je; for(je=key.size()-1; je>=0; je--) if( key[je] == '$' ) break;
    if( je <= js ) return key;
    string result; for(int j=js+1; j<je; j++) result += key[j];
    return result;
}

int egsGetEndian() {
    int nl = 0x12345678; unsigned char *p = (unsigned char *)(&nl);
...
bool egsIsAbsolutePath(const string &path) {
    ...
    if( (path[0] == '/' || path[0] == __egs_fs) &&
        (path[1] == '/' || path[1] == __egs_fs) ) return true;
    if( path.size() < 3 ) return false;
    if( !isalpha(path[0]) ) return false;
    if( path[1] != ':' ) return false;
    if( path[2] != '/' && path[2] != __egs_fs ) return false;
    return true;
#else
    return (path[0] == '/');
#endif
