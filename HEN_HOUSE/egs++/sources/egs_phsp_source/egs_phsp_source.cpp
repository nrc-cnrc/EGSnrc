...
EGS_PhspSource::EGS_PhspSource(const string &phsp_file,
    ...
    init(); openFile(phsp_file);
}

void EGS_PhspSource::init() {
    otype = "EGS_PhspSource";
    recl = 0; Xmin = -1e30; Xmax = 1e30; Ymin = -1e30; Ymax = 1e30;
    is_valid = false; record = 0; mode2 = false; swap_bytes = false;
    the_file_name = "no file"; filter_type = -1; particle_type = 2;
    description = "Invalid phase space source";
    Nread = 0; count = 0; Nrecycle = 0;
    Npos = 0; Nlast = 0;
    wmin = -1e30; wmax = 1e30;
    Nreuse_g = 1; Nreuse_e = 1; Nreuse = 1; Nuse = 2; first = true;
}

void EGS_PhspSource::openFile(const string &phsp_file) {
    if( the_file.is_open() ) the_file.close();
    the_file_name = "no file"; is_valid = false; recl = 0;
    ...
    if( !the_file.is_open() ) {
        egsWarning("EGS_PhspSource::openFile: failed to open binary file %s"
                " for reading\n",phsp_file.c_str()); return;
    }
    string cmode; char auxc;
    for(int i=0; i<5; i++) {
        the_file.get(auxc);
    ...
    if( cmode == "MODE0" ) { mode2 = false; recl = 28; }
    else if( cmode == "MODE2" ) { mode2 = true; recl = 32; }
    else {
        egsWarning("EGS_PhspSource::openFile: the file %s is not a MODE0 or"
                " MODE2 file\n",phsp_file.c_str()); return;
    }
    record = new char [recl];
    int n, n_photon; float emax, emin, pinc;
    ...
    if( n <= 0 || n_photon < 0 || n_photon > n || emin < 0 || emax < 0 ||
        emax < emin || pinc < 0 ) {
        swap_bytes = true;
        egsSwapBytes(&n); egsSwapBytes(&n_photon); egsSwapBytes(&emin);
        egsSwapBytes(&emax); egsSwapBytes(&pinc);
        if( n <= 0 || n_photon < 0 || n_photon > n || emin < 0 || emax < 0 ||
            emax < emin || pinc < 0 ) {
            ...
            if( n_photon < 0 ) egsWarning("  number of photons: %d\n",n_photon);
            if( n_photon > n ) egsWarning("  number of photons (%d) is "
                 "greater than number of particles (%d)\n",n_photon,n);
            if( emin < 0 ) egsWarning("  minimum energy: %g\n",emin);
            if( emax < 0 ) egsWarning("  maximum energy: %g\n",emax);
            if( emin > emax ) egsWarning("  emin > emax: %g %g\n",emin,emax);
            if( pinc < 0 ) egsWarning("  incident particles: %g\n",pinc);
            recl = 0; return;
        }
    }
    ...
    istream::off_type nend = n; nend = nend*recl;
    ...
    the_file.clear(); the_file.seekg(recl,ios::beg);
    Npos = 0; Nlast = n; Nfirst = 1;
    // at this point the position should be at the first particle in the file
    Emax = emax; Emin = emin; Pinc = pinc; Nparticle = n; Nphoton = n_photon;
    is_valid = true; the_file_name = phsp_file;
}

EGS_PhspSource::EGS_PhspSource(EGS_Input *input, EGS_ObjectFactory *f) :
    EGS_BaseSource(input,f) {
    init();
    string fname; int err = input->getInput("phase space file",fname);
    if( err ) {
    ...
    vector<int> the_filter; vector<string> the_latch;
    err = input->getInput("filter type",the_filter);
    int err1 = input->getInput("latch setting",the_latch);
    if( !err && !err1 ) {
        if( the_filter[0] >= 0 && the_filter[0] <= 3 ) {
            int nbit1 = the_latch[0].size(); int nbit2 = 0;
            if( !the_filter[0] ) nbit2 = the_latch[1].size();
            if( nbit1 + nbit2 > 0 ) {
                int *the_bits = new int [nbit1+nbit2];
                for(int j=0; j<nbit1+nbit2; j++) {
                    if( j < nbit1 ) the_bits[j]=(int) (the_latch[0][j] == '1');
                    else the_bits[j] = (int) (the_latch[1][j-nbit1] == '1');
                }
    ...
    int ntmp; err = input->getInput("reuse photons",ntmp);
    if( !err && ntmp > 0 ) Nreuse_g = ntmp;
    err = input->getInput("reuse electrons",ntmp);
    if( !err && ntmp > 0 ) Nreuse_e = ntmp;
    description = "Phase space source from "; description += the_file_name;
}
...
EGS_I64 EGS_PhspSource::getNextParticle(EGS_RandomGenerator *, int &q,
    ...
    */
    if( Nuse >= Nreuse ) readParticle();
    x.x = p.x; x.y = p.y; x.z = 0;
    u.x = p.u; u.y = p.v;
    EGS_Float aux = p.u*p.u+p.v*p.v;
    if( aux < 1 ) aux = sqrt(1-aux); else aux = 0;
    if( p.wt > 0 ) { u.z = aux; wt = p.wt; } else { u.z = -aux; wt = -p.wt; }
    if( rejectParticle() ) wt = 0;
    E = p.E; q = p.q; latch = 0; //latch = p.latch;
    ++Nuse;
    return count;
}
...
void EGS_PhspSource::readParticle() {
    if( (++Npos) > Nlast ) {
        egsWarning("EGS_PhspSource::readParticle(): reached the end of the "
               ...
               Nlast,Nfirst);
        the_file.seekg(recl,ios::beg); Nrecycle++; Npos = Nfirst;
    }
    the_file.read(record,recl*sizeof(char)); ++Nread;
    if( the_file.eof() || !the_file.good() )
        egsFatal("EGS_PhspSource::readParticle(): I/O error while reading "
                "phase space file\n");
    __egs_data32 *data;
    data = (__egs_data32 *) &record[0];  p.latch = data->i;
    data = (__egs_data32 *) &record[4];  p.E = data->f;
    data = (__egs_data32 *) &record[8];  p.x = data->f;
    data = (__egs_data32 *) &record[12]; p.y = data->f;
    data = (__egs_data32 *) &record[16]; p.u = data->f;
    data = (__egs_data32 *) &record[20]; p.v = data->f;
    data = (__egs_data32 *) &record[24]; p.wt = data->f;
    if( swap_bytes ) {
        egsSwapBytes(&p.latch); egsSwapBytes(&p.E); egsSwapBytes(&p.wt);
        egsSwapBytes(&p.x); egsSwapBytes(&p.y);
        egsSwapBytes(&p.u); egsSwapBytes(&p.v);
    }
    if( p.latch & 1073741824 ) p.q = -1;
    else if( p.latch & 536870912 ) p.q = 1;
    else p.q = 0;
    if( p.E < 0 ) { count++; p.E = -p.E; }
    else if( first ) ++count;
    first = false;
    if( p.q ) { p.E -= 0.5110034; Nreuse = Nreuse_e; }
    else Nreuse = Nreuse_g;
    Nuse = 0; p.wt /= Nreuse;
}

bool EGS_PhspSource::rejectParticle() const {
    if( particle_type < 2 && p.q != particle_type ) return true;
    if( particle_type == 3 && !p.q ) return true;
    if( p.x < Xmin || p.x > Xmax || p.y < Ymin || p.y > Ymax ) return true;
    if( p.wt < wmin || p.wt > wmax ) return true;
    if( p.latch & 2147483648UL ) return true;
    if( filter_type < 0 ) return false;
    if( filter_type == 0 ) {
        bool r1; if( filter1 ) r1 = !(p.latch & filter1); else r1 = false;
        bool r2 = (p.latch & filter2);
        return (r1 || r2);
    }
    if( filter_type == 1 ) return (p.latch & filter1);
    int l = p.latch >> 24; bool res = l & filter1;
...
void EGS_PhspSource::setFilter(int type, int nbit1, int nbit2, const int *bits){
    ...
    int ntot = nbit1; if( type == 0 ) ntot += nbit2;
    if( ntot > 29 ) {
    ...
    filter_type = type;
    if ( filter_type == 0 ) {
        int i; filter1 = 0; filter2 = 0;
        for(i=0; i<ntot; i++) {
            if( bits[i] ) {
                unsigned int aux = 1; aux = (aux << i);
                if( i < nbit1 ) filter1 += aux; else filter2 += aux;
            }
        }
    }
