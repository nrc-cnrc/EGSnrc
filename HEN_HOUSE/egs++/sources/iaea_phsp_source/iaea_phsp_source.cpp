...
void IAEA_PhspSource::init() {
    ...
    is_valid = false; mode2 = false; swap_bytes = false;
...
void IAEA_PhspSource::openFile(const string &phsp_file) {
    the_file_name = "no file"; is_valid = false;
    ...
    EGS_I64 n, n_photon, pinc; float emax, zposn;
    ...
    mode2=false;
    for (int i=0; i< n_extra_floats; i++) { if (extrafloat_types[i]==3) { mode2=true; i_zlast=i; break; } }

    //determine array index of latch
    latch_stored=false;
    for (int i=0; i< n_extra_longs; i++) { if (extralong_types[i]==2) { latch_stored=true; i_latch=i; break; } }
    if (!latch_stored) {
    ...
    Emax = emax; Pinc = pinc; Nparticle = n; Nphoton = n_photon;
...
IAEA_PhspSource::IAEA_PhspSource(EGS_Input *input, EGS_ObjectFactory *f) :
    ...
    init();
    string fname; int err = input->getInput("iaea phase space file",fname);
    if( err ) {
    ...
    description = "IAEA phase space source from "; description += the_file_name;
}
...
EGS_I64 IAEA_PhspSource::getNextParticle(EGS_RandomGenerator *, int &q,
    ...
    */
    ...
    if( Nuse >= Nreuse ) { //get a new particle
      ...
      if(!latch_stored) p.latch = 0; //need some non-nonsense value in case there is a LATCH filter
      //note: we don't have to convert to p.E to K.E. because that's what IAEA format stores
      if(first || nstat>0) count++; //increment primary history counter
      first= false;
      //store Nreuse
      if(p.q) Nreuse = Nreuse_e;
