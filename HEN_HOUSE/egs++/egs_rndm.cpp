...
void EGS_RandomGenerator::allocate(int n) {
    if( n < 1 ) egsFatal("Attempt to construct a RNG with n < 1\n");
    rarray = new EGS_Float[n]; np = n; ip = np;
}
...
void EGS_RandomGenerator::copyBaseState(const EGS_RandomGenerator &r) {
    ...
    if( np <= 0 ) allocate(r.np);
    ip = r.ip; have_x = r.have_x; the_x = r.the_x; count = r.count;
    for(int j=ip; j<np; j++) rarray[j] = r.rarray[j];
}

EGS_RandomGenerator::EGS_RandomGenerator(int n) : count(0), np(0) {
    allocate(n);
    have_x = false;
    for(int j=0; j<np; j++) rarray[j] = 0;
}
...
public:
    ...
    int  rngSize() const { return baseSize() + 102*sizeof(int); };

    void setHighResolution(bool hr) { high_res = hr; };
...
protected:
    ...
    void set(const EGS_Ranmar &r) {
        copyBaseState(r);
        ix = r.ix; jx = r.jx; c = r.c;
        iseed1 = r.iseed1; iseed2 = r.iseed2;
        for(int j=0; j<97; j++) u[j] = r.u[j];
    };
...
void EGS_Ranmar::saveState() {
    if( copy ) copy->set(*this);
    else copy = new EGS_Ranmar(*this);
    copy->copy = 0;
}
...
EGS_RandomGenerator* EGS_Ranmar::getCopy() {
    ...
    c->np = 0; c->copy = 0; c->copyBaseState(*this);
    return c;
}
...
void EGS_Ranmar::setState(EGS_RandomGenerator *r) {
    ...
    if( !r1 ) egsFatal("EGS_Ranmar::setState: attampt to set my state by a non EGS_Ranmar RNG!\n");
    ix = r1->ix; jx = r1->jx; c = r1->c;
    iseed1 = r1->iseed1; iseed2 = r1->iseed2;
    for(int j=0; j<97; j++) u[j] = r1->u[j];
}

bool EGS_Ranmar::storePrivateState(ostream &data) {
    data << ix << " " << jx << " " << c << " "
         << iseed1 << " " << iseed2 << " " << high_res << endl;
    for(int j=0; j<97; j++) data << u[j] << " ";
    data <<  endl;
    return data.good();
}
...
EGS_RandomGenerator* EGS_RandomGenerator::createRNG(EGS_Input *input,
    ...
    EGS_Input *i; bool delete_it = false;
    if( input->isA("rng definition") ) i = input;
    else {
    ...
    if( i->compare(type,"ranmar") ) {
        ...
        else res = new EGS_Ranmar(1802,9373+sequence);
        vector<string> hr_options;
        hr_options.push_back("no"); hr_options.push_back("yes");
...
EGS_RandomGenerator* EGS_RandomGenerator::defaultRNG(int sequence) {
    sequence += 97;
    int ixx = 33; int iaux = sequence/30081;
