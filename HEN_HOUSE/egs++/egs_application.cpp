...
EGS_Application::EGS_Application(int argc, char **argv) : input(0), geometry(0),
    ...
    //
    is_pegsless= false;
    if(!getArgument(argc,argv,"-p","--pegs-file",pegs_file)) is_pegsless= true;
...
int EGS_Application::outputData() {
    ...
    */
    ...
    for(int j=0; j<a_objects_list.size(); ++j) {
        if (!a_objects_list[j]->storeState(*data_out)) return 6;
    }
    return 0;
}
...
int EGS_Application::readData() {
    ...
    */
    ...
    for(int j=0; j<a_objects_list.size(); ++j) {
        if (!a_objects_list[j]->setState(*data_in)) return 6;
    }
    return 0;
}
...
int EGS_Application::addState(istream &data) {
    ...
    for(int j=0; j<a_objects_list.size(); ++j)
        if (!a_objects_list[j]->addState(data)) return 5;
    return 0;
}
