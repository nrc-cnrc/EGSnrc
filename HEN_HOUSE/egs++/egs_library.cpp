...
EGS_PrivateLibrary::EGS_PrivateLibrary(const char *lib_name, const char *path) {
  ...
  if(path) {
      fname = path;
      if( fname[fname.size()-1] != fs ) fname += fs;
  }
  fname += lib_prefix;  fname += lib_name; fname += lib_suffix;
#ifdef LIB_DEBUG
...
EGS_Library::~EGS_Library() { delete pl; }

bool EGS_Library::load() { return pl->load(); }

void *EGS_Library::resolve(const char *symb) { return pl->resolve(symb); }

bool EGS_Library::unload() { return pl->unload(); }

bool EGS_Library::isLoaded() const { return (bool) pl->lib; }

bool EGS_Library::autoUnload() const { return pl->au; }

void EGS_Library::setUnload(bool u) { pl->au = u; }

const char *EGS_Library::libraryName() const { return pl->name.c_str(); }

const char *EGS_Library::libraryFile() const { return pl->fname.c_str(); }

void *EGS_Library::resolve(const char *lname, const char *func,
        const char *path) {
    EGS_PrivateLibrary p(lname,path); p.au = false;
    if( !p.load() ) return 0;
