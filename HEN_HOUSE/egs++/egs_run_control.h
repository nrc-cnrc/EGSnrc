...
inline void addRunControlBlock(shared_ptr<EGS_InputStruct> blockPtr) {
    ...
    runBlock->addSingleInput("calculation", false, "The calculation type: first (default, runs a new simulation), resume (continue a crashed simulation), analyze (prints results), combine (combines results from a parallel run). Defaults to 'first'.", {"first", "resume", "analyze", "combine"});
}
