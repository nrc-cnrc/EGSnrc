...
EGS_2DPolygon::EGS_2DPolygon(vector<EGS_2DVector> &points, bool Open) {
    ...
    for(j=0; j<n-2; j++) {
        ...
        if( fabs(aa%bb) < 1e-6 ) {
            pp[j+1] = pp[j+2];
            for(int i=j+2; i<n; i++) pp[i] = pp[i+1];
            n--;
        }
    }
    np = n; p = new EGS_2DVector [np];
    for(j=0; j<n; j++) p[j] = pp[j];
    delete [] pp;
    // now make line normals pointing inwards.
    a = new EGS_2DVector [np-1]; d = new EGS_Float [np-1];
    uj = new EGS_2DVector [np-1];
    pc = new bool [np-1];
    xmin = 1e30, xmax = -1e30, ymin = 1e30, ymax = -1e30;
    for(j=0; j<np-1; j++) {
        pc[j] = true;
        if( p[j].x < xmin ) xmin = p[j].x;
        if( p[j].x > xmax ) xmax = p[j].x;
        if( p[j].y < ymin ) ymin = p[j].y;
        if( p[j].y > ymax ) ymax = p[j].y;
        uj[j] = p[j+1]-p[j];
        if( ccw ) a[j] = EGS_2DVector(-uj[j].y,uj[j].x);
        else a[j] = EGS_2DVector(uj[j].y,-uj[j].x);
        a[j].normalize();
        d[j] = a[j]*p[j];
    }
    // now see if the polygon is convex
    is_convex = true;
    if( np == 4 ) return; // triangles are always convex.
    open = false; // we can not have open polygons
    for(j=0; j<np-1; j++) {
        int j1 = j+1; if( j1 == np-1 ) j1=0;
        bool is_ok = true;
        for(int i=0; i<np-1; i++) {
            if( i != j && i != j1 ) {
                if( !inside(j,p[i]) ) { is_ok=false; break; }
            }
        }
        if( !is_ok ) { is_convex = false; pc[j] = false; } //break; }
    }
    ...
    for(j=0; j<np-1; j++) {
        bool is_ok = true;
        for(int i=0; i<np; i++) {
            if( i < j || i > j+1 ) if( !inside(j,p[i]) ) { is_ok=false; break; }
        }
        if( is_ok ) { // found the point
            jstart = j;
            pp = new EGS_2DVector [np]; int jj=0; int i;
            for(i=j; i<np-1; i++) pp[jj++] = p[i];
            for(i=0; i<=j; i++) pp[jj++] = p[i];
            break;
        }
    }
    ...
    chull.push_back(pp[0]); chull.push_back(pp[1]); int nc=2;
    j=2; bool doing_chull=true;
    while(1) {
        ...
        for(int i=j+1; i<n; i++) {
            if( sperp*pp[i] < ds ) { all_inside=false; break; }
        }
        if( all_inside ) {
            if( !doing_chull ) {
                ipol.push_back(pp[j]);
    ...
    for(j=0; j<ncut; j++) cut[j] = tmp_pol[j];
    delete [] pp;
}
