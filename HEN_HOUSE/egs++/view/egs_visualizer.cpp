...
public:
    ...
    void setLight(int ilight, EGS_Light *light) {
        if( ilight < nlight ) {
            delete lights[ilight]; lights[ilight] = light;
        }
        else addLight(light);
    };
    void addLight(EGS_Light *light) {
        if( nlight == ntot ) {
            int nnew = ntot + 10; EGS_Light **l = new EGS_Light * [nnew];
            for(int j=0; j<nlight; j++) l[j] = lights[j];
            if( ntot > 0 ) delete [] lights;
            ntot = nnew; lights = l;
        }
        lights[nlight++] = light;
    };
    ...
    void addClippingPlane(EGS_ClippingPlane *p) {
        if( nclip >= nclip_t ) {
            ...
            for(int j=0; j<nclip; j++) tmp[j] = clip[j];
            if( nclip_t > 0 ) delete [] clip;
            clip = tmp; nclip_t += 5;
        }
        clip[nclip++] = p;
    };
    ...
    void setMaterialColor(int imed, const EGS_MaterialColor &Mat) {
        if( imed >= nmat ) {
            ...
            for(int j=0; j<nmat; j++) m[j] = mat[j];
            if( nmat > 0 ) delete [] mat;
            mat = m; nmat = imed+5;
        }
        mat[imed] = Mat;
    };
	...
	if(m_tracks) m_tracks->setParticleVisibility(particle, vis);
    }
...
EGS_GeometryVisualizer::~EGS_GeometryVisualizer() { delete p; }
...
void EGS_GeometryVisualizer::addLight(const EGS_Vector &pos,
        const EGS_Vector &color) { p->addLight(pos,color); }
...
void EGS_GeometryVisualizer::setLight(int j, const EGS_Vector &pos,
        const EGS_Vector &color) { p->setLight(j,pos,color); }
...
void EGS_GeometryVisualizer::setMaterialColor(int imed,
        const EGS_MaterialColor &Mat) { p->setMaterialColor(imed,Mat); }
    ...
    if (nclip > 0) {
        ...
        if( jclip >= 0 ) {
            if( tclip <= tleft ) {
                ...
                if( ireg >= 0 ) {
                    imed = g->medium(ireg);
                    if( imed >= 0 ) c = mat[imed].d*mat[imed].alpha;
                    // save region
...
EGS_Vector EGS_PrivateVisualizer::getColor ( const EGS_Vector &x,
    ...
    EGS_Float   t = u.length(); u *= (1./t);    // normalize direction vector
        ...
        for (int j=0; j<nclip; j++) {
            ...
            if (clip[j]->isInside(xo)) {                                        // we are inside the clipping plane
                ...
                if (clip[j]->howfar(xo,u,tt)) {                                 // calculate distance to clipping plane
                    ...
                    if (ttrack>0) return cTrack;                              // don't forget the axis!
                    return c;
                }
            }

            else {                                                              // we are outside clipping plane
                if (clip[j]->howfar(xo,u,t)) tleft = t;                         // remaining distance is that of the clipping plane
            }
        }
    ...
    if (ireg < 0) {
        ...
        if( debug ) egsWarning("enter: inew=%d t=%g imed=%d n=(%g,%g,%g)\n", inew,t,imed,n.x,n.y,n.z);
    ...
    // translucent points
    if( debug ) egsWarning("c=(%g,%g,%g) a=%g\n",c.x,c.y,c.z,a);
    do {
        EGS_Vector  n;
        int imed_new;
        t = tleft;
    ...
    if (ttrack>0) return c+(cTrack)*a;                                           // draw the axis if not hit before
    return c;
}
...
bool EGS_PrivateVisualizer::renderImage(EGS_BaseGeometry *g, int nx, int ny, EGS_Vector *image) {
    ...
    for(int j=0; j<ny; j++) {
        ...
        for(int i=0; i<nx; i++) {
            ...
            if( image ) {
                ...
                if (image[idx].z<0) {
                    ...
                    if ((image[idx].x > 0) || (image[idx].y > 0) && m_tracks) {
                        ...
                        if (image[idx].x == 3.0) { bCol.x=0; bCol.y=0.0; bCol.z=1.0; }
                        if (image[idx].x == 4.0) { bCol.x=1; bCol.y=1; bCol.z=1; }
                        if (image[idx].x == 100.0) { bCol.x=1; bCol.y=1; bCol.z=1; }
                    }
            ...
            if( debug ) egsWarning("ix=%d iy=%d v=(%g,%g,%g)\n",i,j,v.x,v.y,v.z);
            if( v.x > rmax ) rmax = v.x;
            if( v.y > gmax ) gmax = v.y;
            if( v.z > bmax ) bmax = v.z;
            if( image ) image[i+j*nx] = v;
        }
    }
    ...
    if( image ) {
        if( rmax > 1 || gmax > 1 || bmax > 1 ) {
            ...
            for(int j=0; j<nx*ny; j++) image[j].scale(aux);
        }
    }
    return true;
}
...
void EGS_PrivateVisualizer::normalizeImage(EGS_Vector *image, int n) {
    EGS_Float rmax=1, gmax=1, bmax=1;
    for(int j=0; j<n; j++) {
        if( image[j].x > rmax ) rmax = image[j].x;
        if( image[j].y > gmax ) gmax = image[j].y;
        if( image[j].z > bmax ) bmax = image[j].z;
    }
    if( rmax > 1 || gmax > 1 || bmax > 1 ) {
        EGS_Vector aux(1/rmax,1/gmax,1/bmax);
        for(int j=0; j<n; j++) image[j].scale(aux);
    }
}

EGS_PrivateVisualizer::~EGS_PrivateVisualizer() {
    if( nlight > 0 ) {
        for(int j=0; j<nlight; j++) delete lights[j];
        delete [] lights;
    }
    if( nmat  > 0 ) delete [] mat;
    if( nclip_t > 0 ) {
        for(int j=0; j<nclip; j++) delete clip[j];
        delete [] clip;
    }
}
...
void EGS_PrivateVisualizer::setProjection(const EGS_Vector &camera_pos,
                    const EGS_Vector &camera_look_at, EGS_Float distance,
                    EGS_Float size_x, EGS_Float size_y) {
    xo = camera_pos; EGS_Vector u(camera_look_at - xo); u.normalize();
    x_screen = xo + u*distance;
    v2_screen = EGS_Vector(0,u.z,-u.y); v2_screen.normalize();
...
void EGS_PrivateVisualizer::setProjection(const EGS_Vector &camera_pos,
            const EGS_Vector &Xo_screen, const EGS_Vector &V1_screen,
            const EGS_Vector &V2_screen,EGS_Float size_x, EGS_Float size_y) {
    xo = camera_pos; x_screen = Xo_screen;
    v1_screen = V1_screen; v2_screen = V2_screen;
    sx = size_x; sy = size_y;
    if (m_tracks) {
...
bool EGS_GeometryVisualizer::makePngImage(EGS_BaseGeometry *g,
    ...
    delete [] image; png_ostream->close(); delete png_ostream;
    for(int j=0; j<ysize; j++) delete rows[j];
    delete [] rows;
