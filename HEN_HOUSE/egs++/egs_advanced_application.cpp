...
static void __help_get_data(EGS_I32 *nbin,EGS_Float *emin, EGS_Float *emax,
    ...
    __ihelp = nbin; __help1 = emin; __help2 = emax; __help3 = a; __help4 = b;
}

static void __init_interpolator(int q, int imed, int type,
                                EGS_Interpolator &i) {
    if( q ) egsGetElectronData(__help_get_data,&imed,&type);
    else egsGetPhotonData(__help_get_data,&imed,&type);
...
static EGS_LOCAL void __egs_iovar(int nio, int ns,
    ...
    int j;
    for(j=0; j<ns; j++) var[j] = source[j];
    var[ns] = 0;
    for(j=ns+1; j<nio; j++) var[j] = ' ';
}
...
void EGS_LOCAL __write_to_fortran_file(const char *msg, ...) {
    ...
    vsprintf(__write_buf,msg,ap); va_end(ap);
...
int EGS_AdvancedApplication::initEGSnrcBackEnd() {
    ...
    if( run ) the_egsio->n_chunk = run->getNchunk();
...
public:
    ...
    void getInput(EGS_Input *input) {
        if( type == 1 ) {
            EGS_Float aux; int err = input->getInput(name,aux);
            if( !err ) *float_input = aux;
        }
        else if( type == 0 ) {
            EGS_I32 aux; int err = input->getInput(name,aux);
            if( !err ) *int_input = aux;
        }
        else if( type == 3 ) {
            int err = input->getInput(name,charinp);
            if( !err ) {
                int l = charinp.size(); if( l > len ) l = len; int j;
                for(j=0; j<l; j++) char_input[j] = charinp[j];
                for(j=l; j<len; j++) char_input[j] = ' ';
            }
            else{charinp = string(char_input).substr(0,len);}
        }
        else if( type == 4 ) {
            int err = input->getInput(name,*str_v);
            if( !err ) {
              vector<string> str = *str_v;str_v->clear();
              int number = str.size();
              if(number>nitem){
                str.erase(str.begin()+nitem,str.end());number=nitem;
              }
              for(int i=0; i<number; i++){
                if (str[i].size()>len) str[i] = str[i].substr(0,len);
              }
              *str_v = str;
            }
        }
    ...
    void info(int nc) {
        ...
        for(int j=name.size(); j<nc; j++) egsInformation(" ");
...
int EGS_AdvancedApplication::initCrossSections() {
    ...
    if( !transportp ) transportp = input->getInputItem("transport parameter");
...
int EGS_AdvancedApplication::helpInit(EGS_Input *transportp, bool do_hatch) {
    ...
    int nmed_old = nmed; nmed = nmed_new;
    int *ind = new int [nmed];
    for(j=0; j<nmed; j++) {
        const char *medname = geometry->getMediumName(j);
        int len = strlen(medname); ind[j] = egsAddMedium(medname, len);
    }
    ...
    //
    if( the_xoptions->spin_effects != 0 ) the_xoptions->spin_effects=1;
    ...
    pxsec_out.addOption("Off"); pxsec_out.addOption("On");
    ...
    bc.addOption("Off"); bc.addOption("On"); bc.addOption("Simple");
    bc.addOption("norej");
    EGS_TransportProperty radc("Radiative Compton corrections",
            &the_xoptions->radc_flag);
    radc.addOption("Off"); radc.addOption("On");
    ...
    photonuc.addOption("Off"); photonuc.addOption("On");

    EGS_TransportProperty rayl("Rayleigh scattering",&the_xoptions->iraylr);
    rayl.addOption("Off"); rayl.addOption("On"); rayl.addOption("custom");
    ...
    brem.addOption("BH"); brem.addOption("NIST"); brem.addOption("NRC");
    EGS_TransportProperty bang("Brems angular sampling",&the_xoptions->ibrdst);
    bang.addOption("Simple"); bang.addOption("KM");
    EGS_TransportProperty pair("Pair cross sections",&the_xoptions->pair_nrc);
    pair.addOption("BH"); pair.addOption("NRC");
    EGS_TransportProperty pang("Pair angular sampling",&the_xoptions->iprdst);
    pang.addOption("Off"); pang.addOption("Simple"); pang.addOption("KM");
    pang.addOption("Uniform"); pang.addOption("Blend");
    EGS_TransportProperty tran("Electron-step algorithm",
            &the_etcontrol->transport_algorithm);
    tran.addOption("EGSnrc"); tran.addOption("PRESTA-I");
    tran.addOption("PRESTA-II"); tran.addOption("default");
    ...
    if( transportp ) {
        ...
        eii.getInput(transportp);setEIIData(eii.size());
    ...
    if( do_hatch ) {
        ...
        if( nmed_old > 0 ) {
            delete [] i_ededx; delete [] i_pdedx; delete [] i_esig;
            delete [] i_psig; delete [] i_ebr1; delete [] i_pbr1;
            delete [] i_pbr2;
            delete [] i_gmfp; delete [] i_gbr1; delete [] i_gbr2;
            delete [] i_cohe;
        }
        if( nmed > 0 ) {
            ...
            for(int imed=0; imed<nmed; imed++) {
                int type; int imed1 = imed+1;
                type = 3; __init_interpolator(1,imed1,type,i_ededx[imed]);
                type = 4; __init_interpolator(1,imed1,type,i_pdedx[imed]);
                type = 1; __init_interpolator(1,imed1,type,i_esig[imed]);
                type = 2; __init_interpolator(1,imed1,type,i_psig[imed]);
                type = 5; __init_interpolator(1,imed1,type,i_ebr1[imed]);
                type = 6; __init_interpolator(1,imed1,type,i_pbr1[imed]);
                type = 7; __init_interpolator(1,imed1,type,i_pbr2[imed]);
                type = 1; __init_interpolator(0,imed1,type,i_gmfp[imed]);
                type = 2; __init_interpolator(0,imed1,type,i_gbr1[imed]);
                type = 3; __init_interpolator(0,imed1,type,i_gbr2[imed]);
                type = 4; __init_interpolator(0,imed1,type,i_cohe[imed]);
                type = 5; __init_interpolator(0,imed1,type,i_photonuc[imed]);
            }
        }
    }
    ...
    if( !data_ok ) { delete [] ind; return 3; }

    egsInformation("\n\nTransport parameter and cross section options:\n"
                       "==============================================\n");
    int nc = 50;
    if( !isspace(the_media->pxsec[0]) ) pxsec.info(nc);
    if( !isspace(the_media->compxsec[0]) ) cxsec.info(nc);
    pcut.info(nc); pair.info(nc); pang.info(nc); trip.info(nc);
    bc.info(nc); radc.info(nc); rayl.info(nc); relax.info(nc); iphter.info(nc);
    photonuc.info(nc);photonucxsec.info(nc);
    egsInformation("\n");
...
{
    string str_eii,off,on; the_xoptions->eii_flag = 1;string ik="ik";
    for (int i=0; i<len; i++){
       str_eii += toupper(the_media->eiixsec[i]);
       off+=' '; on+=' ';
    }
    off[0]='O';off[1]='F';off[2]='F'; on[0]='O'; on[1]='N';
    if      (str_eii == off) the_xoptions->eii_flag = 0;
    else if (str_eii == on) strcpy(the_media->eiixsec,ik.c_str());
}
...
int EGS_AdvancedApplication::shower() {
    ...
    the_stack->x[0] = p.x.x; the_stack->y[0] = p.x.y; the_stack->z[0] = p.x.z;
    the_stack->u[0] = p.u.x; the_stack->v[0] = p.u.y; the_stack->w[0] = p.u.z;
    the_stack->dnear[0] = 0; the_stack->wt[0] = p.wt;
    the_stack->ir[0] = p.ir + 2; the_stack->iq[0] = p.q;
    the_stack->latch[0] = p.latch; the_stack->np = 1;
#ifdef GDEBUG
    steps_n = 0;
#endif
    egsShower();
...
int EGS_AdvancedApplication::outputData() {
    ...
    if( err ) return err;
    EGS_I32 np, ip; egsGetRNGPointers(&np,&ip);
    if( np < 1 ) return 11;
    if( np > 10000000 ) {
    ...
    (*data_out) << "  " << np << "  " << ip << endl;
    for(int j=0; j<np; j++) (*data_out) << array[j] << " ";
    (*data_out) << endl;
    double ch_steps, all_steps; egsGetSteps(&ch_steps,&all_steps);
...
int EGS_AdvancedApplication::readData() {
    ...
    EGS_Float *array = new EGS_Float [np];
    for(int j=0; j<np; j++) (*data_in) >> array[j];
    if( !data_in->good() ) return 13;
...
int EGS_AdvancedApplication::addState(istream &data) {
    ...
    EGS_Float *array = new EGS_Float [np];
    for(int j=0; j<np; j++) data >> array[j];
    if( !data.good() ) return 13;
    egsSetRNGState(&ip,array);
    delete [] array;
    double ch_steps, all_steps;
    ...
    ch_steps += ch_steps_old; all_steps += all_steps_old;
    egsSetSteps(&ch_steps,&all_steps);
...
EGS_I64 EGS_AdvancedApplication::randomNumbersUsed() const {
    ...
    if( ip <= np ) nused -= (np+1-ip);
    return nused;
}

void EGS_AdvancedApplication::appInformation(const char *msg) {
    if( !msg ) return;
    if( !io_flag ) EGS_Application::appInformation(msg);
    else {
        int ounit = the_egsio->i_log;
...
void EGS_AdvancedApplication::appWarning(const char *msg) {
    if( !msg ) return;
    if( !io_flag ) EGS_Application::appWarning(msg);
    else {
        int ounit = the_egsio->i_log;
        F77_OBJ_(egs_write_string,EGS_WRITE_STRING)(&ounit,msg,strlen(msg));
        checkDeviceFull(stdout);
...
void EGS_AdvancedApplication::appFatal(const char *msg) {
    if( !io_flag ) EGS_Application::appFatal(msg);
    else {
        if( msg ) {
            int ounit = the_egsio->i_log;
            F77_OBJ_(egs_write_string,EGS_WRITE_STRING)(&ounit,msg,strlen(msg));
...
void EGS_AdvancedApplication::getElectronSteps(double &ch_steps,
        double &all_steps) const { egsGetSteps(&ch_steps,&all_steps); }
...
void EGS_AdvancedApplication::startNewParticle() {
    if( geometry->hasRhoScaling() ) {
        ...
        the_useful->rhor = rho; the_useful->rhor_new = rho;
    }
    else { the_useful->rhor = 1; the_useful->rhor_new = 1; }
}
...
void EGS_AdvancedApplication::enterNewRegion() {
    ...
    else the_useful->rhor_new = 1;
}

void EGS_AdvancedApplication::saveRNGState() {
    rndm->saveState();
    int np,ip; egsGetRNGPointers(&np,&ip);
    if( n_rng_buffer > 0 ) {
        ...
        rng_buffer = new EGS_Float [np]; n_rng_buffer = np;
    }
    i_rng_buffer = ip;
...
extern __extc__ void egsHowfar() {
    ...
    if( ireg < 0 || the_stack->wt[np] == 0 ) { the_epcont->idisc = 1; return; }
    int newmed; EGS_Float tsave = the_epcont->ustep;
...
extern __extc__ void egsAusgab(EGS_I32 *iarg) {
    ...
    app->top_p.E = the_stack->E[np]; app->top_p.wt = the_stack->wt[np];
    ...
    app->top_p.q = the_stack->iq[np]; app->top_p.ir = the_stack->ir[np]-2;
    app->top_p.latch = the_stack->latch[np]; app->Np = the_stack->np-1;
...
extern __extc__ void egsStartParticle() {
    ...
    if( ir < 0 ) { the_epcont->idisc = 1; return; }
    the_epcont->idisc = 0;
