...
void EGS_AliasTable::clear() {
    if( n > 0 ) {
        delete [] fi; delete [] xi; delete [] wi; delete [] bin; n = 0;
    }
}

void EGS_AliasTable::allocate(int N, int Type) {
    clear();
    n = N; type = Type; xi = new EGS_Float [n];
    if( type == 0 ) {
        np = n;
        fi = new EGS_Float [n]; wi = new EGS_Float [n]; bin = new int [n];
    }
    else {
        np = n-1;
        wi = new EGS_Float [n-1]; bin = new int [n-1];
        if( type == 1 ) fi = new EGS_Float [n-1];
        else fi = new EGS_Float [n];
    }
}
...
void EGS_AliasTable::copy(const EGS_AliasTable &t) {
    ...
    if( t.n > 0 ) {
        allocate(t.n,t.type);
        for(int j=0; j<np; j++) {
            xi[j] = t.xi[j]; fi[j] = t.fi[j];
            wi[j] = t.wi[j]; bin[j] = t.bin[j];
        }
        if( type ) {
            xi[np] = t.xi[np]; if( type == 2 || type == 3) fi[np] = t.fi[np];
        }
    }
}
...
void EGS_AliasTable::initialize(int N, const EGS_Float *x,
    ...
    if( Type ) xi[np] = x[np];
    if( Type == 2 || Type == 3) fi[np] = f[np];
    make();
}

void EGS_AliasTable::make() {
    EGS_Float *fcum = new EGS_Float[np]; bool *not_done = new bool[np];
    EGS_Float sum = 0, sum1 = 0; int i;
    for(i=0; i<np; i++) {
...
int EGS_AliasTable::initialize(EGS_Float xmin, EGS_Float xmax, EGS_Float accu,
    ...
    allocate(2,2);
    xi[0] = xmin; xi[1] = xmax; fi[0] = f(xi[0],data); fi[1] = f(xi[1],data);
    EGS_Float *xtemp, *ftemp; int error = 0;
    while(1) {
        int nnn = (n-1)*AT_NCHECK + n;
        xtemp = new EGS_Float[nnn]; ftemp = new EGS_Float[nnn];
        if( !xtemp || !ftemp ) egsFatal("EGS_AliasTable::initialize: "
                "not enough memory!\n");
        bool is_ok = true; int i,j=0;
        for(i=0; i<n-1; i++) {
            xtemp[j] = xi[i]; ftemp[j] = fi[i];
            ...
            for(int l=0; l<AT_NCHECK; l++) {
                ...
                if( test > accu ) {
                    is_ok = false; xtemp[++j] = x; ftemp[j] = fe;
                }
            }
            j++;
        }
        if( is_ok ) break;
        xtemp[j] = xi[n-1]; ftemp[j++] = fi[n-1];
        allocate(j,2);
        for(i=0; i<j; i++) { xi[i] = xtemp[i]; fi[i] = ftemp[i]; }
        if( n >= nmax ) { error = 1; break; }
        delete [] xtemp; delete [] ftemp;
    }
    delete [] xtemp; delete [] ftemp;
    make();
    return error;
...
int EGS_AliasTable::sampleBin(EGS_RandomGenerator *rndm) const {
    ...
    EGS_Float aj = r1*np; int j = (int) aj; aj -= j;
    if( aj > wi[j] ) j = bin[j];
    return j;
}
...
EGS_Float EGS_AliasTable::sample(EGS_RandomGenerator *rndm) const {
    ...
    if( aj > wi[j] ) j = bin[j];
    if( !type ) return xi[j];
    EGS_Float x = xi[j]; EGS_Float dx = xi[j+1] - x;
    EGS_Float r2 = rndm->getUniform();
    if( type == 1 ) return x + dx*r2;
    EGS_Float res;
    if( fi[j] > 0 ) {
        EGS_Float a = fi[j+1]/fi[j]-1;
        if( fabs(a) < 0.2 ) {
            EGS_Float rnno1 = 0.5*(1-r2)*a; res = x + r2*dx*(1+rnno1*(1-r2*a));
        }
        else res = x - dx/a*(1-sqrt(1+r2*a*(2+a)));
    }
    else res = x + dx*sqrt(r2);
    return res;
}
...
EGS_SimpleAliasTable::EGS_SimpleAliasTable(int N, const EGS_Float *f) : n(0) {
    if( N < 1 ) return;
    n = N;
    wi = new EGS_Float [n]; bins = new int [n]; int i;
    ...
    for(i=0; i<n-1; ++i) {
        ...
        if( fcum[jh_old] < sum && jh_old < jl_old ) jl = jh_old;
        else {
