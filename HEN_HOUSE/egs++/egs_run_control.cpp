...
EGS_RunControl::EGS_RunControl(EGS_Application *a) : app(a), input(0),
    ...
    if( err ) geomErrorMax = 0;
    vector<string> ctype;
    ctype.push_back("first"); ctype.push_back("restart");
    ctype.push_back("analyze"); ctype.push_back("combine");
...
EGS_RunControl::~EGS_RunControl() {
    ...
    if( !n_run_controls ) {
        while( rc_libs.size() > 0 ) {
            delete rc_libs[rc_libs.size()-1]; rc_libs.pop_back();
        }
    }
}

bool EGS_RunControl::storeState(ostream &data) {
    if( !egsStoreI64(data,ndone) ) return false;
...
bool EGS_RunControl::setState(istream &data) {
    EGS_I64 ndone1;
    if( !egsGetI64(data,ndone1) ) return false;
    ndone += ndone1; ncase += ndone1;
...
public:
...
#ifndef WIN32
        fl_write.l_type = F_WRLCK; fl_write.l_whence = SEEK_SET;
        fl_write.l_start = 0; fl_write.l_len = 0;
        fl_unlock.l_type = F_UNLCK; fl_unlock.l_whence = SEEK_SET;
        fl_unlock.l_start = 0; fl_unlock.l_len = 0;
#endif
    };
    ~EGS_FileLocking() { if( fd > 0 ) CLOSE_FILE(fd); };
    ...
    bool openControlFile(const char *fname) {
        ...
        for(int t=0; t<ntry; t++) {
            fd = OPEN_FILE(fname,OPEN_FLAGS); if( fd > 0 ) break;
            WAIT_FOR_FILE;
        }
        return ( fd > 0 );
    };
    bool closeControlFile() {
        if( fd > 0 ) {
            int res = CLOSE_FILE(fd); fd = -1; return !res;
        }
        return true;
    };
    bool lockControlFile() {
        ...
        if( !res ) { is_locked = true; return true; }
        return false;
#else
...
#endif
        if( !res ) { is_locked = false; return true; }
        return false;
    };
    bool rewindControlFile() {
        if( fd < 0 ) return false;
        if( !is_locked ) {
            if( !lockControlFile() ) return false;
        }
#ifdef WIN32
...
EGS_JCFControl::EGS_JCFControl(EGS_Application *a, int Nbuf) :
    ...
    if( nbuf < 0 ) nbuf = 1024; buf = new char [nbuf];
    nleft = ncase; ntot = 0;
...
bool EGS_JCFControl::writeControlString() {
    ...
    if( first_time ) start_time = time(0);
    /*
    MY_OSTREAM data(buf);
    //ostream &data = cout;
...
EGS_I64 EGS_JCFControl::getNextChunk() {
    if( !readControlFile() ) return -1;
    if( first_time ) { first_time = false; njob++; }
    double sum, sum2, count;
    app->getCurrentResult(sum,sum2,norm,count);
    tsum += sum - last_sum; tsum2 += sum2 - last_sum2;
    tcount += count - last_count;
    last_sum = sum; last_sum2 = sum2; last_count = count;
    EGS_I64 nrun = ncase/(npar*nchunk);
    if( nrun < 1 ) nrun = 1;
    if( nrun > nleft ) nrun = nleft;
    if( nrun > 0 ) app->setSimulationChunk(ntot,nrun);
    nleft -= nrun; ntot += nrun;
    writeControlFile();
    double f,df;
...
int EGS_RunControl::finishSimulation() {
    ...
    double ch_steps, all_steps; app->getElectronSteps(ch_steps,all_steps);
...
int EGS_JCFControl::finishSimulation() {
    ...
    if( err < 0 ) return err;
    if( removed_jcf ) return 0;
    if( !readControlFile() ) return -2;
    njob--;
    writeControlFile(); p->closeControlFile();
    if( njob > 0 || removed_jcf ) return 0;
    string cfile = egsJoinPath(app->getAppDir(),app->getFinalOutputFile());
    cfile += ".lock";
#ifdef WIN32
...
EGS_JCFControl::~EGS_JCFControl() { delete p; }

bool EGS_JCFControl::closeControlFile() { return p->closeControlFile(); }

bool EGS_JCFControl::lockControlFile() { return p->lockControlFile(); }

bool EGS_JCFControl::unlockControlFile() { return p->unlockControlFile(); }

bool EGS_JCFControl::rewindControlFile() { return p->rewindControlFile(); }
...
EGS_RunControl* EGS_RunControl::getRunControlObject(EGS_Application *a) {
    ...
    EGS_Input *inp = a->getInput(); EGS_Input *irc = 0;
    if( inp ) irc = inp->getInputItem("run control");
    if( !irc ) {
        /*
        egsWarning("EGS_RunControl::getRunControlObject(): "
        ...
        return 0;
        */
        if( a->getNparallel() > 0 ) return new EGS_JCFControl(a);
        else return new EGS_RunControl(a);
    }
    /*
    ...
    */
    string libname; int err = irc->getInput("library",libname);
    ...
    if( !err ) {
        ...
        if( !lib ) {
            ...
            if( !lib->isLoaded() ) {
                egsWarning("EGS_RunControl::getRunControlObject: failed to"
                    ...
                    dsodir.c_str()); delete irc; return 0;
            }
        ...
        if( !create ) {
            egsWarning("EGS_RunControl::getRunControlObject: failed to"
                   ...
                   libname.c_str()); result = 0;
        }
        else result = create(a);
    }
    else {
        if( a->getNparallel() > 0 ) result = new EGS_JCFControl(a);
        else result = new EGS_RunControl(a);
    }
    delete irc; return result;
}
