...
extern __extc__ void EGS_EXPORT F77_OBJ_(egspp_init_geometry,EGSPP_INIT_GEOMETRY)(
    ...
    int n = flength-1;
    while( isspace(file_name[n]) && n > 0 ) --n;
    char *fname = new char [n+1];
    for(int j=0; j<n; ++j) fname[j] = file_name[j];
    fname[n] = '\0';
    EGS_Input input; input.setContentFromFile(fname);
    delete [] fname;
    EGS_BaseGeometry *g = EGS_BaseGeometry::createGeometry(&input);
    if( g ) {
        *igeom = egspp_f_geoms.size();
        g->ref(); egspp_f_geoms.add(g);
    }
    else {
...
extern __extc__ void EGS_EXPORT F77_OBJ_(egspp_get_medium_name,EGSPP_GET_MEDIUM_NAME)(
    ...
    if( n > mlength ) n = mlength;
    int j;
    for(j=0; j<n; ++j) medname[j] = the_medium[j];
    for(j=n; j<mlength; ++j) medname[j] = ' ';
}
