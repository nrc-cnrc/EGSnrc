...
EGS_TriangleShape::EGS_TriangleShape(const vector<EGS_Float> &points,
    ...
    xo = points[0]; yo = points[1];
    ax = points[2] - xo; ay = points[3] - yo;
    bx = points[4] - xo; by = points[5] - yo;
...
EGS_PolygonShape::EGS_PolygonShape(const vector<EGS_Float> &points,
    const string &Name, EGS_ObjectFactory *f) : EGS_SurfaceShape(Name,f) {
    int np = points.size(); n = np/2;
    EGS_Float auxx = points[np-2] - points[0];
    EGS_Float auxy = points[np-1] - points[1];
    EGS_Float *xc, *yc;
    if( auxx*auxx + auxy*auxy > 1e-8 ) n++;
    xc = new EGS_Float [n]; yc = new EGS_Float [n];
    vector<EGS_2DVector> p1;
    for(int j=0; j<np/2; j++) {
        xc[j] = points[2*j]; yc[j] = points[2*j+1];
        p1.push_back(EGS_2DVector(xc[j],yc[j]));
    }
    if( n > np/2 ) {
        xc[n-1] = points[0]; yc[n-1] = points[1];
        p1.push_back(EGS_2DVector(xc[n-1],yc[n-1]));
    }
    EGS_2DPolygon pol(p1);
    int ntr = 0; triangle = new EGS_TriangleShape* [n-3]; np = n;
    EGS_Float p_tmp[6];
    while ( np > 3 ) {
        for(int i=0; i<np-2; i++) {
            EGS_2DVector aux(0.5*(xc[i+2]+xc[i]),0.5*(yc[i+2]+yc[i]));
            if( pol.isInside(aux) ) {
                bool is_ok = true; vector<EGS_2DVector> p2;
                for(int k=0; k<3; k++) {
                    p_tmp[2*k] = xc[i+k]; p_tmp[2*k+1] = yc[i+k];
                ...
                for(int j=0; j<np-1; j++) {
                    if( j < i || j > i+2 ) {
                        ...
                        if( tri.isInside(tmp) ) { is_ok = false; break; }
                    }
                }
                if( is_ok ) {
    ...
    for(int k=0; k<3; k++) { p_tmp[2*k] = xc[k]; p_tmp[2*k+1] = yc[k]; }
    triangle[ntr++] = new EGS_TriangleShape(p_tmp);
    A = 0;
    for(int i=0; i<ntr; i++) { yc[i] = triangle[i]->area(); A += yc[i]; }
...
EGS_POLYGON_SHAPE_EXPORT EGS_BaseShape* createShape(EGS_Input *input,
    ...
    EGS_BaseShape *shape;
    if( np == 6 ) shape = new EGS_TriangleShape(points,"",f);
    else shape = new EGS_PolygonShape(points,"",f);
