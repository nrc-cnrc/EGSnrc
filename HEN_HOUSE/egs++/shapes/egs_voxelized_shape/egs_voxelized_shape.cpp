...
void EGS_VoxelizedShape::EGS_VoxelizedShapeFormat0(const char *fname,
    ...
    prob=0; xpos=0; ypos=0; zpos=0; map=0; nx=0; ny=0; nz=0; nxy=0; nreg=0; type=-1;
    ...
    char endian, form;
    data.read(&endian,1); data.read(&form,1);
    if( data.fail() ) {
    ...
    if( form == 0 ) {
        ...
        if( data.fail() ) {
            ...
            delete [] p; delete [] x; delete [] y; delete [] z; return;
        }
        nmap = nreg;
    }
    else {
        ...
        if( data.fail() ) {
            ...
            delete [] p; delete [] map; delete [] x; delete [] y; delete [] z; return;
        }
    }
    EGS_Float *p1 = new EGS_Float [nmap]; int j;
    for(j=0; j<nmap; ++j) p1[j] = p[j];
    delete [] p;
    ...
    for(j=0; j<=nx; ++j) xpos[j] = x[j];
    for(j=0; j<=ny; ++j) ypos[j] = y[j];
    for(j=0; j<=nz; ++j) zpos[j] = z[j];
...
EGS_VoxelizedShape::EGS_VoxelizedShape(int file_format, const char *fname,
    ...
    while(1) {
        ...
        if ( key ==  "matrix size [1]" ) { sscanf(value.c_str(), "%u", &Nx); }
            ...
            else if( (value == "signed integer") || (value == "SIGNED INTEGER")) data_type = 2;
    ...
    {
        int j;
        for(j=0; j<=nx; j++) x[j] = -(float)nx*scale_x/2.0 + j*scale_x;
        for(j=0; j<=ny; j++) y[j] = -(float)ny*scale_y/2.0 + j*scale_y;
        for(j=0; j<=nz; j++) z[j] = -(float)nz*scale_z/2.0 + j*scale_z;
    }
    ifstream i_file(data_file.c_str(),ios::binary);
    if( !i_file ) {
        egsWarning("%s: failed to open interfile data "
                "%s\n",func,data_file.c_str()); return;
    }
    int nmap = nreg;
    ...
    if (data_type == 0) {
        ...
        for (int cc = 0; cc<nreg; cc++) p[cc] = (float) (p_tmp[cc]);
        delete [] p_tmp;
    } else {
    ...
    for(j=0; j<=nx; ++j) xpos[j] = x[j];
    for(j=0; j<=ny; ++j) ypos[j] = y[j];
    for(j=0; j<=nz; ++j) { zpos[j] = z[j]; egsInformation("%f ", zpos[j]); }
    delete [] x; delete [] y; delete [] z;
    type = 0;
    egsInformation("Done\n");
}
...
EGS_VOXELIZED_SHAPE_EXPORT EGS_BaseShape* createShape(EGS_Input *input,
    ...
    string fname; int err = input->getInput("file name",fname);
    int file_format; int err2 = input->getInput("file format",file_format);
    if( err ) {
