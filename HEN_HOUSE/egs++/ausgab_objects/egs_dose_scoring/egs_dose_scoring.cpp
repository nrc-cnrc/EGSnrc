...
void EGS_DoseScoring::setApplication(EGS_Application *App) {
    ...
    if (score_region_dose){
       if (d_region.size() && d_region.size() < nreg){// specific dose regions provided
          ...
          if (score_region_dose) dose = new EGS_ScoringArray(nreg);
       }
    }
    // If requested request memory for medium dose
    if (score_medium_dose) doseM =  new EGS_ScoringArray(nmedia);
    if (!score_region_dose){// setup volumes
        if ( vol_list.size() == 1) vol.push_back(vol_list[0]);
        else{
          for (int j=0; j<nreg; j++){
              if (j < vol_list.size())
               vol.push_back(vol_list[j]);
    ...
    if (norm_u != 1.0){
      ...
      sprintf(buf,"%g\n",norm_u); description += buf;
    }
    description += "\n*******************************************\n\n";
    vol_list.clear(); if (d_region.size()) d_region.clear();
}
...
void EGS_DoseScoring::reportResults() {
    ...
    normE = m_lastCase/F*norm_u; normD = 1.602e-10*normE;
...
EGS_DOSE_SCORING_EXPORT EGS_AusgabObject* createAusgabObject(EGS_Input *input,
    ...
    /* get dose regions */
    vector <int> d_regions; bool using_all_regions=true;
    ...
    if (!err04) result->setUserNorm(norma);
    return result;
}
