...
void BEAM_DoseScoring::setApplication(EGS_Application *App) {
    ...
    for(int i=0; i<d_cms.size(); i++) {
       if (d_cms[i]==0) d_start = 0;
       else d_start = bapp->CM_reg_end(d_cms[i]-1)+1;
    ...
    if (d_region.size()){// specific dose regions provided
       ...
       if (score_region_dose) dose = new EGS_ScoringArray(d_region.size());
    }
    // If requested request memory for medium dose in each CM
    if (score_medium_dose) doseM =  new EGS_ScoringArray(nmedia*d_cms.size());
    ...
    description += "\n--------------------------------------\n";
    sprintf(buf,"%*s %*s rho/[g/cm**3]\n",max_medl/2,"medium",max_medl/2," ");description += buf;
    description += "--------------------------------------\n";
    for ( imed=0; imed < nmedia; imed++){
      sprintf(buf,"%-*s",max_medl,app->getMediumName(imed));description += buf;description += " ";
      sprintf(buf,"%5.2f",app->getMediumRho(imed));description += buf;description += "\n";
    }
    description += "\n*******************************************\n\n";
    if (d_region.size()) d_region.clear();
}
...
void BEAM_DoseScoring::reportResults() {
    ...
    normE = m_lastCase/F; normD = 1.602e-10*normE;
    int irmax_digits = getDigits(max_dreg);
    string line; double r,dr;
    if (dose){
        if (normE==1){
    ...
    if (doseM){
        vector<EGS_Float> massM(d_cms.size()*nmedia,0); int imed = 0;
...
bool BEAM_DoseScoring::storeState(ostream &data) const {
  ...
  if( !egsStoreI64(data,m_lastCase)) return false;
  data << endl;
  if( dose  && !dose->storeState(data))  return false;
  if( doseM && !doseM->storeState(data)) return false;
  return true;
}

bool  BEAM_DoseScoring::setState(istream &data){
  if( !egsGetI64(data,m_lastCase) ) return false;
  if( dose  && !dose->setState(data))  return false;
  if( doseM && !doseM->setState(data)) return false;
  return true;
}
...
int  BEAM_DoseScoring::addTheStates(istream &data){
   EGS_I64 tmp_case; if( !egsGetI64(data,tmp_case) ) return 4401;
   m_lastCase += tmp_case;
   if (dose){
        EGS_ScoringArray tmp(nreg);
        if( !tmp.setState(data) ) return 4402;
        (*dose) += tmp;
   }
   if (doseM){
     EGS_ScoringArray tmpM(nmedia);
     if( !tmpM.setState(data) ) return 4403;
     (*doseM) += tmpM;
   }
   return 0;
}
...
BEAM_DOSE_SCORING_EXPORT EGS_AusgabObject* createAusgabObject(EGS_Input *input,
    ...
    allowed_mode.push_back("no");allowed_mode.push_back("yes");
      ...
      if(!err2 && !err3) {
       if(vreg_start.size()==vreg_stop.size()) {
         if(v_in.size()<vreg_start.size()) {
           ...
           for(int i=v_in.size()+1; i<vreg_start.size(); i++) v_in.push_back(v_in[v_in.size()-1]);
         }
         for(int i=0; i<vreg_start.size(); i++) {
           if(volin.size()<=vreg_stop[i]) {
            //enlarge to volin array
            for(int j=volin.size(); j<=vreg_stop[i]; j++) volin.push_back(1.0);
           }
           for(int j=vreg_start[i]; j<=vreg_stop[i]; j++) volin[j]=v_in[i];
         }
       }
       else {
    ...
    if(volin.size()) result->setVol(volin);
    else result->setVol(1.0);
    result->setDoseCMs(d_cms);
    if ( d_in_medium ) result->setMediumScoring(true);
    if ( d_in_region ) result->setRegionScoring(true);
