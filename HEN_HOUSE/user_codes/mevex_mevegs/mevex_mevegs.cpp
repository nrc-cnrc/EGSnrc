...
public:
 std::string iApplicationInputFile;

 void setMesh(TetrahedralMesh &aMesh){
 	pMesh = &aMesh;
 };
    ...
    Mevegs_Application(int argc, char **argv):
        EGS_AdvancedApplication(argc,argv){
        std::cout << "Successfully constructed MevEGS Application" << std::endl;
        if (this->input_file.compare(0,2,".\\") == 0){
      		iApplicationInputFile = this->input_file.substr(2,this->input_file.size()-2);
      	} else{
      		iApplicationInputFile = this->input_file;
      	}
    };
    ...
    EGS_Mevegs_Mesh* getGeometry() const {
       return dynamic_cast<EGS_Mevegs_Mesh*>(geometry);
    }
...
int Mevegs_Application::initScoring() {
  ...
  auto err = initPhaseSpaceScoring();
  if (err) {
    egsInformation("error in phase space scoring initialization\n");
  }

  // initialize nreg
  this->nreg = geometry->regions();
  this->score = new EGS_ScoringArray(nreg+2); // + reflected and transmitted
  the_egsvr->i_do_rr = 1;
...
int Mevegs_Application::initGeometry(){
    ...
    if( mevegs ) {
      if(!mevegs->getInput("scaling", scaling) && scaling != 0) {
        egsInformation(" => initScoring: scaling by a factor of %lf\n", scaling);
      }
      delete mevegs;
    }

    if(scaling != scaling) {
      egsInformation("Error scaling, got NaN! Using default of %lf.\n", default_scaling);
      scaling = default_scaling;
    }

    geometry = createMeshGeometry(input, this, scaling, *pMesh);

    egsInformation("Mesh class\n");
...
int Mevegs_Application::initPhaseSpaceScoring() {
    // parse geometry section to check for phase space scoring geometries
    if (!EGS_BaseGeometry::createGeometry(this->input)) {
        return 1;
    }
    return 0;
}
...
void Mevegs_Application::write_gmsh() {
    ...
    auto result_file = std::ofstream(this->getMesh().output_filename(), std::ios::app);
    ...
    auto extra_info = std::ofstream(this->getMesh().output_filename() + ".egsinfo", std::ios::app);
...
{
    auto elt_tags = this->getMesh().element_tags();
...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < argc; i++) {
      ...
      if ((strArg.size() > 3) && (strArg.compare(strArg.size()-4, 4, ".msh")) == 0) {
      	if (strArg.compare(0,2,".\\") == 0){
      		meshFilePath = strArg.substr(2,strArg.size()-2);
      	} else{
      		meshFilePath = strArg;
      	}
      }
    ...
    auto mesh = gmsh_manip::mesh_from_gmsh(meshFilePath,app.iApplicationInputFile);
    app.setMesh(mesh);
