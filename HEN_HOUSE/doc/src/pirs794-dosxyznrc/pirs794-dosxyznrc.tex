...
latent variance
if the phase space source is allowed to rewind the phase space file
...
rewinding the phase space file should be avoided.
\indexm{NRCYCL}
...
before the requested number of histories is run, then DOSXYZnrc rewinds
...
be re-used whenever a phase space source is rewinded
(happens automatically when the simulation has reached
the end of the source).  Rewinding is not recommended, however,
...
prevents the source from being rewinded.
...
the phase space source will rewind.  This may be due to particles
...
sampled adequately.
If the
source has only been rewinded once and only a small fraction of it
...
if the source rewinds, calculate {\tt NRCYCL} using Equation~\ref{nrcycleqn}
...
{\tt NRCYCL} to avoid rewinding a phase space source also has
...
would be rewinded at least once during the run.  After rewinding, particle
